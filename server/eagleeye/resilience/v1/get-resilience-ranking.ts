...
} from '../../../../src/generated/server/eagleeye/resilience/v1/service_server';
...
async function isSeedRefreshAuthorized(ctx: ServerContext): Promise<boolean> {
  ...
  const candidate = ctx.request.headers.get('X-EagleEye-Key') ?? '';
...
export const getResilienceRanking: ResilienceServiceHandler['getResilienceRanking'] = async (
  ...
  // (Pro bearer, EAGLEEYE_VALID_KEYS, EAGLEEYE_API_KEY) must keep the
