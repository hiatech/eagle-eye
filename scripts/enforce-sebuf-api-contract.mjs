 ...
 *      generated service_server under src/generated/server/eagleeye/<snake_domain>/v<N>/.
...
const GEN_SERVER_DIR = join(ROOT, 'src/generated/server/eagleeye');
...
//      e.g. /api/v2/shipping/*).
// Both map to src/generated/server/eagleeye/<snake_domain>/v<N>/.
  ...
  if (gatewayMatch) {
    ...
    if (!existsSync(expectedServer)) {
      violation(
        ...
        `Expected ${relative(ROOT, expectedServer)}. Either regenerate (cd proto && buf generate), restore the proto under proto/eagleeye/${snakeDomain}/v${version}/service.proto, or delete this orphaned gateway.`,
      );
    }
    continue;
  }
