 ...
 *
 * Requires: Referer header from trusted origin OR X-EagleEye-Key header.
 * Uses api.eagle-eye.app by default.
 */

const BASE_URL = process.argv.includes('--base-url')
  ? process.argv[process.argv.indexOf('--base-url') + 1]
  : 'https://api.eagle-eye.app';
...
const API_KEY = process.env.EAGLEEYE_KEY || '';
...
async function fetchEndpoint(endpoint) {
  ...
  const resp = await fetch(url, {
    headers: {
      ...
      ...(API_KEY ? { 'X-EagleEye-Key': API_KEY } : {}),
    },
