...
const SANDBOX_NOTE =
  ...
  'Guide: https://www.eagle-eye.app/docs/sandbox';
...
export function buildSandboxFixtures(repoRoot = root) {
  ...
  for (const path of SANDBOX_OPERATIONS) {
    ...
    const fixture = {
      ...
      operation: {
        ...
        path,
        summary: op.summary ?? '',
        productionUrl: `https://api.eagle-eye.app${path}`,
    ...
    indexOperations.push({
      ...
      fixture: `https://www.eagle-eye.app/sandbox/${slug}.json`,
  ...
  const index = {
    ...
    product: 'Eagle Eye',
    description:
      'Eagle Eye sandbox: deterministic, schema-valid sample responses for representative REST operations. ' +
      'Fetch any fixture below with plain HTTP — no auth, no quota, safe for CI. Each fixture mirrors the exact ' +
      'envelope the production endpoint returns; switch to productionUrl with an X-EagleEye-Key header to go live.',
    docs: 'https://www.eagle-eye.app/docs/sandbox',
    openapi: 'https://eagle-eye.app/openapi.json',
    authGuide: 'https://eagle-eye.app/auth.md',
