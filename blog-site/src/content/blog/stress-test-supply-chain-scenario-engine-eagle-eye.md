---
title: "Stress-Test Your Supply Chain with EagleEye's Scenario Engine"
...
metaTitle: "Supply Chain Scenario Engine Stress Test | EagleEye"
...
heroImage: "/blog/images/blog/stress-test-supply-chain-scenario-engine-eagle-eye.jpg"
pubDate: "2026-06-13"
...
EagleEye's Scenario Engine is built for that what-if layer. It takes named disruption templates, runs them through the supply-chain exposure graph, and returns the affected chokepoints, sectors, and countries so analysts can compare the projected impact against the live baseline.
...
Scenario Engine sits on top of EagleEye's chokepoint registry, live chokepoint status, HS2 exposure caches, and supply-chain panel state. The template catalog is curated in code. The job queue and worker output are explicit: callers can see pending, processing, done, and failed states rather than receiving an opaque spinner.
...
The template assumptions and execution path are inspectable in the public [scenario-engine source](https://github.com/hiatech/eagle-eye/tree/main/server/eagleeye/scenario) and [scenario template catalog](https://github.com/hiatech/eagle-eye/blob/main/server/eagleeye/supply-chain/v1/scenario-templates.ts).
