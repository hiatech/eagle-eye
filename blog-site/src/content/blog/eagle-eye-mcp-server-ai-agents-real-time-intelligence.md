---
title: "Give Your AI Agent Eyes on the World: The Eagle Eye MCP Server"
...
metaTitle: "Real-Time Intelligence MCP Server | Eagle Eye"
...
The Model Context Protocol (MCP) fixes the plumbing problem: it gives AI assistants a standard way to call live tools. Eagle Eye fixes the data problem: it exposes the entire intelligence platform, the same one behind the [free real-time dashboard](/blog/posts/what-is-eagle-eye-real-time-global-intelligence/), as an MCP server with **59 live tools**.
...
{
  "mcpServers": {
    "eagleeye": { "url": "https://eagle-eye.app/mcp" }
  }
}
```

**Claude web (claude.ai):** Settings → Connectors → Add custom connector → name it EagleEye, paste `https://eagle-eye.app/mcp`.
...
```bash
npx @modelcontextprotocol/inspector https://eagle-eye.app/mcp
```

On first use, the client walks you through "Sign in with EagleEye Pro." MCP access requires a [Pro account](https://www.eagle-eye.app/pro); API Starter and Enterprise keys (`X-EagleEye-Key`) also work for server-to-server agents. OAuth clients do not need copied keys because the protocol handles token exchange and refresh.
...
Every Eagle Eye tool accepts an optional `jmespath` argument. The server applies the expression server-side and returns only the projection:
...
Eagle Eye treats autonomous agents as first-class clients. From the root URL, an agent can discover everything by itself:
...
So an agent that has never heard of Eagle Eye can start from `https://eagle-eye.app/`, read the Link headers, and wire itself up without a human in the loop. If you prefer raw REST over MCP, the same data is available through the [developer API](/blog/posts/build-on-eagle-eye-developer-api-open-source/), and the two share authentication.
...
Eagle Eye follows the official [Model Context Protocol specification](https://modelcontextprotocol.io/specification/latest). Client support and transport behavior can vary, so verify the client's current MCP documentation when configuring production access.
...
**Does the Eagle Eye MCP server work with ChatGPT or other non-Claude clients?**
...
**Add `https://eagle-eye.app/mcp` to your MCP client and ask your agent what changed in the world today. Setup docs at [eagle-eye.app/docs/mcp-quickstart](https://www.eagle-eye.app/docs/mcp-quickstart).**
