---
title: "Build on Eagle Eye: APIs and Developer Platform"
description: "Build intelligence apps on Eagle Eye's typed API: 36 services, 295 proto files, 80+ edge functions, and auto-generated TypeScript clients. AGPL-3.0."
metaTitle: "Developer API & Open Source Platform | Eagle Eye"
...
heroImage: "/blog/images/blog/build-on-eagle-eye-developer-api-open-source.jpg"
pubDate: "2026-03-09"
...
Eagle Eye is designed differently. The entire intelligence platform, every data feed, every scoring algorithm, every aggregation pipeline, is built on a **typed API layer** that developers can use, extend, and build upon.

## Proto-First Architecture

Eagle Eye uses **Protocol Buffers (protobuf)** as the single source of truth for all API contracts. The codebase contains:
...
For developers building on Eagle Eye, this means you can trust the API contracts completely. If the proto says a field is `int64`, it's `int64`. If it says `repeated string`, it's an array of strings.

## 35 Service Domains

Eagle Eye's API is organized into domain-specific services:
...
```
api.eagle-eye.app/api/{domain}/v1/{rpc}
```

For example:

- `api.eagle-eye.app/api/market/v1/quotes` for stock quotes
- `api.eagle-eye.app/api/conflict/v1/events` for conflict data
- `api.eagle-eye.app/api/intelligence/v1/cii` for Country Instability Index scores

## Building with Eagle Eye's API
...
Feed Eagle Eye data into your own analytics:
...
- Build custom scoring models using Eagle Eye's raw data feeds

### Mobile Apps

Build a mobile app that consumes Eagle Eye's API for a custom mobile intelligence experience. The OpenAPI spec makes it accessible from any language (Swift, Kotlin, Python, Go).
...
Every developer surface has a dedicated, named page you can jump to directly — the [Eagle Eye Developer Portal](https://eagle-eye.app/developers.md) links them all in one place:

- **[Eagle Eye MCP Server](https://eagle-eye.app/mcp-server.md)** — the recommended agent surface at `https://eagle-eye.app/mcp`, with 59 tools over Streamable HTTP. Connect Claude, Cursor, or any MCP client. See the [MCP Overview](https://www.eagle-eye.app/docs/mcp-overview) for auth and the full catalog.
- **[Eagle Eye OpenAPI Specification](https://eagle-eye.app/openapi.md)** — the OpenAPI 3.1 contract for the REST API ([openapi.yaml](https://eagle-eye.app/openapi.yaml) / [openapi.json](https://eagle-eye.app/openapi.json)), so you can generate a typed client in any language.
- **[Eagle Eye SDKs](https://eagle-eye.app/sdks.md)** — official zero-dependency client libraries for Python, Ruby, Go, and JavaScript, plus the [`eagleeye` CLI](https://www.eagle-eye.app/docs/cli).
- **Eagle Eye API docs** — the full [developer documentation](https://www.eagle-eye.app/docs/documentation) site, with an [MCP Quickstart](https://www.eagle-eye.app/docs/mcp-quickstart) and [agent auth walkthrough](https://eagle-eye.app/auth.md).

## Self-Hosting

Eagle Eye is AGPL-3.0. You can self-host the entire platform, including [local AI capabilities that run without cloud dependencies](/blog/posts/ai-powered-intelligence-without-the-cloud/):
...
For reference, Eagle Eye is built with:
...
## Why Build on Eagle Eye?

The intelligence industry has a consolidation problem. A handful of vendors control the data, the algorithms, and the interfaces. Analysts are locked into ecosystems they can't customize, audit, or extend. See how Eagle Eye [compares to traditional intelligence tools](/blog/posts/eagle-eye-vs-traditional-intelligence-tools/) in practice.

Eagle Eye's open, typed, proto-first architecture is the alternative:
...
The intelligence platform of the future isn't a product. It's an ecosystem. Eagle Eye is building the foundation.

Building an AI agent instead of an app? The same platform is exposed as a Model Context Protocol server with 59 live tools. See [how to connect Claude and other agents to Eagle Eye's MCP server](/blog/posts/eagle-eye-mcp-server-ai-agents-real-time-intelligence/).

## Frequently Asked Questions

**Is the Eagle Eye API free to use?**
Yes. Eagle Eye is AGPL-3.0 open source. You can use the public API at api.eagle-eye.app or self-host the entire stack. There are no API keys required for public endpoints and no usage fees.
...
**Start building at [github.com/hiatech/eagle-eye](https://github.com/hiatech/eagle-eye). 36 services, 295 proto files, and a global intelligence dataset waiting for your application.**
