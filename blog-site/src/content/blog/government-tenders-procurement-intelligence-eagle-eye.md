...
description: "EagleEye aggregates public tenders from SAM.gov, TED, Contracts Finder, CanadaBuys, GETS, and the World Bank into one searchable procurement feed."
metaTitle: "Global Government Tender Tracking | EagleEye"
...
heroImage: "/blog/images/blog/government-tenders-procurement-intelligence-eagle-eye.jpg"
pubDate: "2026-07-21"
...
The problem is that this signal is scattered across national portals that share no format, no API conventions, and no common vocabulary. EagleEye's Global Procurement panel pulls the major official sources into one searchable feed.
...
EagleEye separates two different questions:
...
- **REST:** `GET /api/economic/v1/list-global-tenders` — paginated up to 100 records per page, with filters for country, region, source, status, buyer, publish and deadline dates, value range, currency, category, and free-text query. See the [API reference](https://www.eagle-eye.app/docs/api-reference).
...
Both paths are part of EagleEye Pro and enforce the subscription server-side. Pair the feed with [country risk screening](/blog/posts/country-risk-monitoring-due-diligence-eagle-eye/) before chasing an opportunity in an unfamiliar market, and with the [tariff and trade-policy trackers](/blog/posts/tariff-tracker-trade-policy-monitoring-eagle-eye/) when the contract involves cross-border delivery.
...
Use the API with a saved query and your own scheduler, or point an MCP-connected agent at `get_procurement_opportunities` on a schedule — the [Slack and Teams alerting guide](/blog/posts/geopolitical-risk-alerts-slack-teams-eagle-eye-api/) shows the pattern.
