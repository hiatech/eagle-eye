...
description: "Connect Claude to EagleEye's MCP server so non-developers can ask live questions about conflicts, markets, chokepoints, country risk, and breaking news."
metaTitle: "Ask Claude About Current Events with EagleEye MCP"
keywords: "Claude connectors real-time news, ask AI about current events, Claude MCP server current events, EagleEye MCP Claude, AI current events assistant"
audience: "Claude users, analysts, founders, journalists, operators, non-developer power users"
heroImage: "/blog/images/blog/eagle-eye-mcp-server-ai-agents-real-time-intelligence.jpg"
pubDate: "2026-06-10"
...
EagleEye's [MCP server](https://www.eagle-eye.app/docs/mcp-overview) gives Claude a live tool connection. You add one connector URL, approve the OAuth flow, and Claude can call the same intelligence surfaces behind the [EagleEye dashboard](/blog/posts/what-is-eagle-eye-real-time-global-intelligence/): country risk, conflicts, markets, maritime chokepoints, aviation, cyber, energy, news intelligence, forecasts, and more.

You do not need to write an app. You do not need to understand JSON-RPC. You only need a paid EagleEye tier and a Claude client that supports custom connectors.
...
EagleEye's MCP server exists for this exact pattern. It exposes 59 tools, six prompt templates, and read-only resources through the [Model Context Protocol](https://modelcontextprotocol.io), so Claude can discover tools and call them during a conversation.
...
```text
https://eagle-eye.app/mcp
```

Use the apex host exactly as shown. The dashboard and docs commonly live at `www.eagle-eye.app`, but the MCP connector endpoint is intentionally published at `eagle-eye.app/mcp`.

For Claude web, open Settings, go to Connectors, add a custom connector named `EagleEye`, and paste that URL.
...
```json
{
  "mcpServers": {
    "eagleeye": {
      "url": "https://eagle-eye.app/mcp"
    }
  }
}
```

Restart Claude Desktop after saving the file. On first use, Claude opens the EagleEye OAuth consent flow. Pro users sign in with their EagleEye account. API Starter, Business, and Enterprise users can use OAuth or an issued `wm_...` API key for server-side workflows.

Free accounts can still use the public dashboard, but the MCP server requires a paid tier. The [MCP quickstart](https://www.eagle-eye.app/docs/mcp-quickstart) keeps the setup details current if a client changes its connector UI.

## Six prompts that work immediately

You can ask plain-English questions, but the fastest way to get reliable answers is to start from repeatable prompt shapes. These are the six templates EagleEye exposes through MCP:
...
```text
Use EagleEye before answering. Give me a 10-minute global brief for today.
...
If Claude answers without calling tools, ask it to retry with EagleEye tools first.
...
```text
Use EagleEye to brief Turkey for an operations team. Include country risk,
...
```text
Use EagleEye to check route risk from the Gulf to Northwest Europe. Look at
...
```text
Use EagleEye to evaluate this claim: "<paste claim>". Return an evidence
...
```text
Use EagleEye for a market-open geopolitical prep. Include oil, gold, major
...
```text
Run a EagleEye freshness audit for the tools you just used. Which fields are
...
- Do not assume EagleEye covers every source on earth.
...
**Can non-developers use EagleEye MCP with Claude?**

Yes. Claude web users can add EagleEye as a custom connector, and Claude Desktop users can add the server URL to the desktop config. The setup is mostly copy-paste; no application code is required.

**Does this give Claude real-time current events?**

It gives Claude access to EagleEye's live and cache-backed intelligence tools. Claude still has to call the tools, preserve freshness fields, and explain uncertainty. Ask it to use EagleEye before answering live-risk questions.

**Is MCP free?**

The public dashboard is free, but EagleEye MCP access requires Pro or an API tier. The MCP docs list the current auth modes, quotas, and client setup details.

---

**Add `https://eagle-eye.app/mcp` as a Claude connector, then ask one narrow operational question: what changed in the world that matters to me today?**
