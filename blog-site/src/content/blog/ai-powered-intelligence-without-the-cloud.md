---
title: "Private AI Intelligence: Eagle Eye Without the Cloud"
description: "Run AI-powered intelligence analysis on your own hardware. Eagle Eye supports Ollama, LM Studio, and in-browser ML for private geopolitical analysis."
metaTitle: "Local AI Intelligence Analysis | Eagle Eye"
...
Eagle Eye takes a different approach. Every AI feature in the platform can run entirely on your own hardware, with no data leaving your machine. If you're new to the platform, learn [what Eagle Eye is and how it works](/blog/posts/what-is-eagle-eye-real-time-global-intelligence/).
...
## Eagle Eye's 4-Tier AI Architecture

Eagle Eye solves this with a **4-tier LLM fallback chain** that starts local and only reaches for the cloud if you explicitly allow it:

### Tier 1: Local LLMs (Ollama / LM Studio)

Your first and most private option. Install Ollama or LM Studio on your machine, download a model (Llama 3.1, Mistral, Phi, etc.), and point Eagle Eye at your local instance.
...
The desktop app ([Tauri](https://tauri.app/)) discovers your local [Ollama](https://ollama.com/) instance automatically. No configuration needed. Just install Ollama, pull a model, and open Eagle Eye.
...
This tier is limited (T5 is smaller than Llama 3.1), but it means Eagle Eye's AI features always work, even without internet access.

## In-Browser Machine Learning

Beyond the LLM tiers, Eagle Eye runs several ML pipelines entirely in your browser:
...
Eagle Eye's **Headline Memory** feature builds a local semantic index of up to 5,000 headlines using ONNX embeddings stored in IndexedDB. When you ask the AI about a topic, it retrieves relevant headlines from your local index for grounded, cited responses.
...
Eagle Eye's Tauri desktop app (available for macOS, Windows, and Linux) takes privacy further:
...
### Step 2: Open Eagle Eye

Navigate to eagle-eye.app or install the desktop app from GitHub releases.

### Step 3: Configure AI

Eagle Eye auto-detects your local Ollama instance. Open any country dossier or the World Brief panel and the AI analysis generates locally.

### Step 4: Enable Headline Memory (Optional)

Opt in to the RAG feature. Eagle Eye will build a local vector index of headlines you've seen, giving the AI context for more grounded analysis.
...
**Government Analysts:**
Classified environments can't send data to commercial AI APIs. Eagle Eye with Ollama runs entirely within your network boundary.
...
You don't have to take our word for the privacy claims. Eagle Eye is fully open source under AGPL-3.0. Every network call, every data flow, every AI prompt is in the codebase for you to audit. Developers can explore the full [typed API layer and proto-first architecture](/blog/posts/build-on-eagle-eye-developer-api-open-source/) to verify exactly how data flows through the system.
...
**Do I need an internet connection to use Eagle Eye's AI features?**
...
**Which local LLM models work best with Eagle Eye?**
...
**Run intelligence analysis on your own terms at [eagle-eye.app](https://www.eagle-eye.app). Install Ollama for fully private AI. No login, no tracking, no compromise.**
