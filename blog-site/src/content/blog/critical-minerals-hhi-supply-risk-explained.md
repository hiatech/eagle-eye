...
description: "How EagleEye uses 2024 production data and the Herfindahl-Hirschman Index to explain supply concentration in lithium, cobalt, rare earths, and more."
metaTitle: "Critical Minerals HHI Supply Risk Explained | EagleEye"
...
EagleEye's supply-chain scoring code labels HHI like this:
...
For each mineral, EagleEye groups production by mineral, sums producer tonnes, computes producer shares, computes HHI, assigns a risk rating, and displays the leading producer countries.
...
That is why EagleEye places mineral concentration next to other supply-chain signals. A critical HHI score plus an export-control headline plus a chokepoint disruption is a different risk profile than a critical HHI score alone.

For operational follow-through, pair this structural concentration view with the [supply-chain scenario engine](/blog/posts/stress-test-supply-chain-scenario-engine-eagle-eye/) and the [maritime chokepoint explainer](/blog/posts/what-is-a-maritime-chokepoint/).

## Source transparency

EagleEye computes the Critical Minerals view from a committed 2024 production dataset and transparent scoring logic. The HHI formula is public in the codebase, and the risk thresholds are simple enough for a spreadsheet check.
