...
const KEYRING_SERVICE: &str = "eagle-eye";
...
fn open_live_channels_window(app: &AppHandle, base_url: Option<String>) -> Result<(), String> {
    ...
    let mut channels_builder = WebviewWindowBuilder::new(app, "live-channels", url)
        .title("Channel management - Eagle Eye")
...
fn build_app_menu(handle: &AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let about_metadata = AboutMetadata {
        name: Some("Eagle Eye".into()),
        version: Some(env!("CARGO_PKG_VERSION").into()),
        copyright: Some("\u{00a9} 2025 Elie Habib".into()),
        website: Some("https://eagle-eye.app".into()),
        website_label: Some("eagle-eye.app".into()),
        ..Default::default()
    };
    let about_item =
        PredefinedMenuItem::about(handle, Some("About Eagle Eye"), Some(about_metadata))?;
...
fn handle_menu_event(app: &AppHandle, event: tauri::menu::MenuEvent) {
    match event.id().as_ref() {
        ...
        MENU_HELP_GITHUB_ID => {
            let _ = open_in_shell("https://github.com/hiatech/eagle-eye");
        }
        #[cfg(feature = "devtools")]
        MENU_HELP_DEVTOOLS_ID => {
...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .expect("error while running eagle-eye tauri application")
        .run(|app, event| {
            match &event {
