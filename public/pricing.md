...
Live tier/price/product-ID data (JSON): `GET https://www.eagle-eye.app/api/product-catalog` — public, no key required. Send a descriptive `User-Agent` (for example `mytool/1.0 (+https://yoursite.example)`); default HTTP-library user agents may be challenged by the edge firewall.

## License scope

- **Pro — Personal license:** one named subscriber may use Eagle Eye for their own research, learning, investing and decisions. It does not cover employer, business or client use.
...
- **API Business — Commercial license for your customers:** the subscribing organization may display, embed or deliver API outputs as part of its own customer-facing product or service. It does not permit raw-data resale, shared API access or a standalone substitute for Eagle Eye.
...
These subscription licenses cover the hosted Service and its outputs. They are separate from the AGPL-3.0-only platform source-code license and the MIT licenses on the official thin clients. Full scope, examples and restrictions: https://eagle-eye.app/docs/terms#subscription-license-scope
...
- Includes: 56 map layers (all free except Resilience, which is Pro), 500+ curated feeds backed by 588+ observed upstream hosts, country briefs, hotspots, instability scores, chokepoints, cables, cascade analysis, breaking alert pipeline and watchlists
...
- License: Commercial license included — use EagleEye for client work, internal tools and reporting
...
- Best for: Developers and teams that want programmatic access to Eagle Eye intelligence data
...
- License: Commercial license for your customers — embed EagleEye data in what you sell; the 5 bundled Pro seats all require the same company email domain
- Limits: 300 requests/minute, 10,000 requests/day
- Upgrading from Starter: manage the switch from the billing portal (prorated immediately); new customers can subscribe directly at https://eagle-eye.app/pro
...
- Per-endpoint request budgets are documented at https://www.eagle-eye.app/docs/usage-rate-limits (also fetchable as markdown at https://www.eagle-eye.app/docs/usage-rate-limits.md).
- Need a higher limit? Upgrade at https://eagle-eye.app/pro or contact enterprise@eagle-eye.app for custom quotas.
...
```json
{
  ...
  "pricing_url": "https://www.eagle-eye.app/pro#pricing",
  "currency": "USD",
  "plans": [
    {
      "name": "Free",
