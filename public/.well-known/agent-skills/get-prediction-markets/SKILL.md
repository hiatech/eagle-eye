...
```
GET https://api.eagle-eye.app/api/prediction/v1/list-prediction-markets
```
...
curl -s --get -H "X-EagleEye-Key: $WM_API_KEY" \
  'https://api.eagle-eye.app/api/prediction/v1/list-prediction-markets' \
...
- For Eagle Eye's own model-generated scenario forecasts, use `GET /api/forecast/v1/get-forecasts`.
- Via MCP, the equivalent tool is `get_prediction_markets` on `https://eagle-eye.app/mcp`.

## References

- OpenAPI: https://eagle-eye.app/openapi.json — operation `ListPredictionMarkets`.
