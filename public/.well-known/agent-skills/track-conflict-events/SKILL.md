...
```
GET https://api.eagle-eye.app/api/conflict/v1/list-ucdp-events
```
...
curl -s --get -H "X-EagleEye-Key: $WM_API_KEY" \
  'https://api.eagle-eye.app/api/conflict/v1/list-ucdp-events' \
...
- Via MCP, the equivalent tool is `get_conflict_events` on `https://eagle-eye.app/mcp`.

## References

- OpenAPI: https://eagle-eye.app/openapi.json — operation `ListUcdpEvents`.
