...
```
GET https://api.eagle-eye.app/api/aviation/v1/list-airport-delays
```
...
curl -s --get -H "X-EagleEye-Key: $WM_API_KEY" \
  'https://api.eagle-eye.app/api/aviation/v1/list-airport-delays' \
...
- Via MCP, the equivalent tool is `get_aviation_status` on `https://eagle-eye.app/mcp`.

## References

- OpenAPI: https://eagle-eye.app/openapi.json — operation `ListAirportDelays`.
