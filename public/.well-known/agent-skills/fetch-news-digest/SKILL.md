...
version: 1
description: Retrieve the pre-aggregated digest of Eagle Eye's 500+ curated news feeds, bucketed by category, with per-article threat classification and alert flags. Use when the user asks what's in the news right now, wants headlines by topic, or needs a current-events sweep.
---

# fetch-news-digest

Use this skill when the user asks what's happening in the news — the latest headlines overall, by category (geopolitics, tech, finance, commodities…), or in a specific language. This is Eagle Eye's core surface: one call returns the aggregated output of 500+ curated RSS feeds, already de-duplicated, categorized, and threat-classified.
...
```
GET https://api.eagle-eye.app/api/news/v1/list-feed-digest
```
...
curl -s --get -H "X-EagleEye-Key: $WM_API_KEY" \
  'https://api.eagle-eye.app/api/news/v1/list-feed-digest' \
...
- Via MCP, the equivalent tool is `get_news_intelligence` on `https://eagle-eye.app/mcp`.

## References

- OpenAPI: https://eagle-eye.app/openapi.json — operation `ListFeedDigest`.
