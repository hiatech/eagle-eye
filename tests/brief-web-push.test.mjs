...
function makeSwSandbox() {
  ...
  const self = {
    location: { origin: 'https://eagle-eye.app' },
...
describe('push-handler.js — push event', () => {
  it('renders a notification with the payload fields', () => {
    ...
    box.emit('push', pushEvent({
      ...
      url: 'https://eagle-eye.app/api/brief/user_abc/2026-04-18?t=xxx',
    ...
    assert.equal(opts.data.url, 'https://eagle-eye.app/api/brief/user_abc/2026-04-18?t=xxx');
  ...
  it('falls back to "EagleEye" title when payload omits it', () => {
    const box = makeSwSandbox();
    loadHandlerInto(box);
    box.emit('push', pushEvent({ body: 'body only, no title' }));
    assert.equal(box.shown[0].title, 'EagleEye');
  });
...
describe('push-handler.js — notificationclick', () => {
  it('opens the target url when no existing window matches', async () => {
    const box = makeSwSandbox();
    loadHandlerInto(box);
    const ev = notifClickEvent({ url: 'https://eagle-eye.app/api/brief/user_a/2026-04-18?t=abc' });
    box.emit('notificationclick', ev);
    assert.equal(ev.closed, true);
    // Wait for the waitUntil chain
    for (const p of ev.waits) await p;
    assert.equal(box.opened, 'https://eagle-eye.app/api/brief/user_a/2026-04-18?t=abc');
  });
  ...
  it('focuses + navigates an existing same-origin window instead of opening', async () => {
    ...
    loadHandlerInto(box);
    const ev = notifClickEvent({ url: 'https://eagle-eye.app/api/brief/u/d?t=t' });
    box.emit('notificationclick', ev);
    for (const p of ev.waits) await p;
    assert.equal(focused, true);
    assert.equal(navigated, 'https://eagle-eye.app/api/brief/u/d?t=t');
