...
describe('createRelayHandler', () => {
  ...
  it('returns CORS headers on every response', async () => {
    ...
    const res = await handler(makeRequest('https://eagle-eye.app/api/test'));
  ...
  it('responds 204 to OPTIONS', async () => {
    const handler = createRelayHandler({ relayPath: '/test' });
    const res = await handler(makeRequest('https://eagle-eye.app/api/test', { method: 'OPTIONS' }));
  ...
  it('responds 405 to non-GET', async () => {
    const handler = createRelayHandler({ relayPath: '/test' });
    const res = await handler(makeRequest('https://eagle-eye.app/api/test', { method: 'POST' }));
  ...
  it('responds 401 when requireApiKey and no valid key', async () => {
    ...
    const res = await handler(makeRequest('https://eagle-eye.app/api/test', {
      headers: { Origin: 'https://tauri.localhost', 'X-EagleEye-Key': 'wrong-key' },
    }));
  ...
  it('allows request when requireApiKey and key is valid', async () => {
    ...
    const handler = createRelayHandler({ relayPath: '/test', requireApiKey: true });
    const res = await handler(makeRequest('https://eagle-eye.app/api/test', {
      headers: { Origin: 'https://tauri.localhost', 'X-EagleEye-Key': 'real-key-123' },
    }));
    assert.equal(res.status, 200);
  });
  ...
  it('proxies relay response with correct status and body', async () => {
    ...
    const res = await handler(makeRequest('https://eagle-eye.app/api/data'));
    assert.equal(res.status, 200);
  ...
  it('forwards search params by default', async () => {
    ...
    const handler = createRelayHandler({ relayPath: '/test' });
    await handler(makeRequest('https://eagle-eye.app/api/test?foo=bar&baz=1'));
  ...
  it('drops search params when forwardSearch is false', async () => {
    ...
    await handler(makeRequest('https://eagle-eye.app/api/test?foo=bar'));
  ...
  it('uses buildRelayPath for dynamic paths', async () => {
    ...
    await handler(makeRequest('https://eagle-eye.app/api/oref?endpoint=history'));
