...
describe('nlweb: /ask endpoint', () => {
  ...
  function post(body, headers = {}) {
    return handler(
      new Request('https://eagle-eye.app/ask', {
        method: 'POST',
  ...
  it('POST {query} returns NLWeb JSON with the _meta envelope', async () => {
    ...
    for (const r of body.results) {
      ...
      assert.equal(r.site, 'eagle-eye.app');
    }
  });
  ...
  it('no-match queries return the honest llms.txt fallback, not a fabricated hit', async () => {
    ...
    assert.equal(body.results[0].url, 'https://eagle-eye.app/llms.txt');
  ...
  it('streaming also triggers via top-level flag, Accept header, and ?prefer.streaming=', async () => {
    for (const req of [
      ...
      handler(new Request('https://eagle-eye.app/ask?query=markets&prefer.streaming=true', { method: 'GET' })),
    ]) {
      const res = await req;
      assert.match(res.headers.get('Content-Type'), /text\/event-stream/);
      await res.text();
  ...
  it('GET with ?query= works for simple probes', async () => {
    const res = await handler(new Request('https://eagle-eye.app/ask?query=sanctions', { method: 'GET' }));
    assert.equal(res.status, 200);
    const body = await res.json();
  ...
  it('query-less probes get a 200 self-describing NLWeb envelope (scanner existence checks read 4xx as absent)', async () => {
    ...
    const bareGet = await handler(new Request('https://eagle-eye.app/ask', { method: 'GET' }));
  ...
  it('unsupported methods → 405; OPTIONS → 204 with CORS', async () => {
    const del = await handler(new Request('https://eagle-eye.app/ask', { method: 'DELETE' }));
    assert.equal(del.status, 405);
    const options = await handler(new Request('https://eagle-eye.app/ask', { method: 'OPTIONS' }));
