...
describe('validateEmail', () => {
  ...
  it('rejects disposable domain (passmail.net)', async () => {
    const result = await validateEmail('eagleeye.foo@passmail.net');
    assert.strictEqual(result.valid, false);
  });
...
describe('parseCsvLine (RFC 4180)', () => {
  ...
  it('parses a Resend data row with angle brackets in from field', () => {
    const row = 'abc-123,2026-03-10,You\'re on the Pro waitlist,Eagle Eye <noreply@eagle-eye.app>,test@gmail.com,,,,bounced,2026-03-10,,key-123';
  ...
  it('handles quoted subject with comma', () => {
    const row = 'abc,"Subject, with comma","Eagle Eye <noreply@eagle-eye.app>",test@example.com,,,,bounced';
    const fields = parseCsvLine(row);
