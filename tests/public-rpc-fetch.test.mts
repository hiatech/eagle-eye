...
describe('publicRpcFetch', () => {
  ...
  it('adds the isolated public cache marker and strips credential-bearing request state', async () => {
    ...
    await publicRpcFetch('https://api.eagle-eye.app/api/news/v1/list-feed-digest?variant=full&lang=en', {
      credentials: 'include',
      headers: {
        Authorization: 'Bearer secret',
        'X-EagleEye-Key': 'wm_secret',
  ...
  it('rejects routes outside the narrow shared-RPC allowlist before fetching', async () => {
    ...
    await assert.rejects(
      publicRpcFetch('https://api.eagle-eye.app/api/intelligence/v1/get-risk-scores'),
