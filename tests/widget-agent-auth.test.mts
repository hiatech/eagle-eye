...
describe('widget-agent unified tester key auth', () => {
  ...
  it('accepts X-EagleEye-Key and upgrades relay request to pro', async () => {
    const res = await handler(new Request('https://www.eagle-eye.app/api/widget-agent', {
      method: 'POST',
      headers: {
    ...
    assert.equal(call.arguments[0], 'https://proxy.eagle-eye.app/widget-agent');
    ...
    assert.equal(headers.get('X-EagleEye-Key'), null);
  ...
  it('falls back to legacy tester keys when X-EagleEye-Key is invalid', async () => {
    const res = await handler(new Request('https://www.eagle-eye.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.eagle-eye.app',
        'Content-Type': 'application/json',
        'X-EagleEye-Key': 'wrong-key',
  ...
  it('rejects invalid X-EagleEye-Key before relay fetch', async () => {
    const res = await handler(new Request('https://www.eagle-eye.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.eagle-eye.app',
  ...
  it('rejects prefix and length mismatches for browser and legacy tester keys', async () => {
    const browserPrefix = await handler(new Request('https://www.eagle-eye.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.eagle-eye.app',
        'Content-Type': 'application/json',
        'X-EagleEye-Key': 'browser-test',
      },
      body: JSON.stringify({ prompt: 'Build a widget', mode: 'create', tier: 'pro' }),
    }));
    assert.equal(browserPrefix.status, 403);

    const proLonger = await handler(new Request('https://www.eagle-eye.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.eagle-eye.app',
        'Content-Type': 'application/json',
    ...
    const widgetLonger = await handler(new Request('https://www.eagle-eye.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.eagle-eye.app',
        'Content-Type': 'application/json',
