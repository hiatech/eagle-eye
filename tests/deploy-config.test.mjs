...
const APP_ROOT_HOST_PATTERN = '^(?:(?:www|tech|finance|commodity|happy|energy)\\.)?eagle-eye\\.app$';
...
describe('crawlable content corpus deployment contracts', () => {
  ...
  it('keeps robots.txt advertising root, blog, and Mintlify docs sitemaps', () => {
    assert.match(robotsSource, /^Sitemap: https:\/\/www\.eagle-eye\.app\/sitemap\.xml$/m);
    assert.match(robotsSource, /^Sitemap: https:\/\/www\.eagle-eye\.app\/blog\/sitemap-index\.xml$/m);
    assert.match(robotsSource, /^Sitemap: https:\/\/www\.eagle-eye\.app\/docs\/sitemap\.xml$/m);
  });
  ...
  it('keeps the root sitemap generated while delegating blog and docs inventories', () => {
    ...
    assert.ok(!sitemapSource.includes('https://www.eagle-eye.app/blog/'));
    assert.ok(!sitemapSource.includes('https://www.eagle-eye.app/docs/'));
  ...
  it('discovers canonical generated corpus pages and validates changelog pagination links', () => {
    ...
    try {
      writeFixturePage(
        ...
        '<link rel="canonical" href="https://www.eagle-eye.app/countries/ukraine/" /><meta name="lastmod" content="2026-07-08" />'
      );
      writeFixturePage(
        publicDir,
        'chokepoints/suez-canal/index.html',
        '<link rel="canonical" href="https://www.eagle-eye.app/chokepoints/suez-canal/" />'
      );
      writeFixturePage(
        publicDir,
        'crises/ukraine-war/index.html',
        '<link rel="canonical" href="https://www.eagle-eye.app/crises/ukraine-war/" />'
      );
      writeFixturePage(
        publicDir,
        'tools/natural-hazard-pulse/index.html',
        '<link rel="canonical" href="https://www.eagle-eye.app/tools/natural-hazard-pulse/" />'
      );
      writeFixturePage(
        publicDir,
        'reference/changelog/page/1/index.html',
        '<link rel="canonical" href="https://www.eagle-eye.app/reference/changelog/page/1/" /><link rel="next" href="https://www.eagle-eye.app/reference/changelog/page/2/" />'
      );
      writeFixturePage(
        publicDir,
        'reference/changelog/page/2/index.html',
        '<link rel="canonical" href="https://www.eagle-eye.app/reference/changelog/page/2/" /><link rel="prev" href="https://www.eagle-eye.app/reference/changelog/page/1/" />'
      );

      const pages = discoverContentCorpusPages({ publicDir });
      const locations = pages.map((page) => page.loc).sort();
      assert.deepEqual(locations, [
        'https://www.eagle-eye.app/reference/changelog/page/1/',
        'https://www.eagle-eye.app/reference/changelog/page/2/',
        'https://www.eagle-eye.app/chokepoints/suez-canal/',
        'https://www.eagle-eye.app/crises/ukraine-war/',
        'https://www.eagle-eye.app/countries/ukraine/',
        'https://www.eagle-eye.app/tools/natural-hazard-pulse/',
      ].sort());

      writeFixturePage(
        publicDir,
        'reference/changelog/page/3/index.html',
        '<link rel="canonical" href="https://www.eagle-eye.app/reference/changelog/page/3/" />'
      );
      assert.throws(
...
describe('deploy/cache configuration guardrails', () => {
  ...
  it('serves /pro hashed assets immutable — broader /pro rules must not override', () => {
    ...
    assert.equal(
      effectiveCacheControl('/pro/assets/eagle-eye-7-mar-2026-abc.jpg'),
      'public, max-age=31536000, immutable'
    );
...
// welcome route is consistent across eagle-eye.app, finance.eagle-eye.app,
// tech.eagle-eye.app, commodity.eagle-eye.app, happy.eagle-eye.app, and
...
describe('welcome landing page routing', () => {
  ...
  it('routes app roots to welcome and leaves non-app roots on the dashboard catch-all', () => {
    assert.equal(rootDestinationForHost('eagle-eye.app'), '/pro/welcome.html');
    assert.equal(rootDestinationForHost('www.eagle-eye.app'), '/pro/welcome.html');
    assert.equal(rootDestinationForHost('eagle-eye.app.evil.example'), DASHBOARD_HTML_DESTINATION);

    const variantHosts = getVariantHosts().filter((host) => host !== 'www.eagle-eye.app');
  ...
  it('keeps variant canonicals aligned with the /dashboard routing strategy', () => {
    ...
    assert.equal(variantUrls.full, 'https://www.eagle-eye.app/dashboard');
  ...
  it('sitemap lists dashboard routes and does not list legacy /welcome', () => {
    ...
    assert.ok(
      sitemap.includes('<loc>https://www.eagle-eye.app/dashboard</loc>'),
      'public/sitemap.xml must list https://www.eagle-eye.app/dashboard'
    );
    for (const host of ['tech', 'finance', 'commodity', 'happy', 'energy']) {
      assert.ok(
        sitemap.includes(`<loc>https://${host}.eagle-eye.app/dashboard</loc>`),
        `public/sitemap.xml must list https://${host}.eagle-eye.app/dashboard`
      );
    }
    assert.ok(
      !sitemap.includes('<loc>https://www.eagle-eye.app/welcome</loc>'),
      'public/sitemap.xml must not list legacy https://www.eagle-eye.app/welcome'
    );
  });
  ...
  it('pins welcome and dashboard SEO canonicals to their new routes', () => {
    ...
    assert.ok(
      welcomeHtml.includes('<link rel="canonical" href="https://www.eagle-eye.app/" />'),
      'welcome source must canonicalize to root'
    );
    assert.ok(
      !welcomeHtml.includes('https://www.eagle-eye.app/welcome'),
      'welcome source must not emit legacy /welcome SEO URLs'
    );
    assert.ok(
      generatedWelcomeHtml.includes('<link rel="canonical" href="https://www.eagle-eye.app/" />'),
      'generated welcome HTML must canonicalize to root'
    );
    assert.ok(
      !generatedWelcomeHtml.includes('https://www.eagle-eye.app/welcome'),
      'generated welcome HTML must not emit legacy /welcome SEO URLs'
    );
    assert.ok(
      generatedWelcomeHtml.includes('https://www.eagle-eye.app/dashboard'),
      'generated welcome HTML must launch the dashboard at /dashboard'
    );
    assert.ok(
      dashboardHtml.includes('<link rel="canonical" href="https://www.eagle-eye.app/dashboard" />'),
  ...
  it('keeps welcome dashboard launch CTAs off the root welcome route', () => {
    ...
    const variantRootWelcomeLaunchLink = /https:\/\/(?:tech|finance|commodity|happy|energy)\.eagle-eye\.app\/\?ref=welcome-/;
    assert.doesNotMatch(
...
describe('security header guardrails', () => {
  ...
  it('CSP frame-src includes Clerk origin for auth modals', () => {
    ...
    assert.ok(
      frameSrc.includes('clerk.accounts.dev') || frameSrc.includes('clerk.eagle-eye.app'),
...
describe('agent readiness: api-catalog + openapi build', () => {
  ...
  const apiEntry = apiCatalog.linkset.find((entry) => entry.anchor === 'https://api.eagle-eye.app/');

  it('linkset[0] is the catalog anchor and enumerates each API via RFC 9727 item links', () => {
    assert.equal(catalogEntry.anchor, 'https://eagle-eye.app/.well-known/api-catalog');
    ...
    assert.ok(itemHrefs.includes('https://api.eagle-eye.app/'), 'item list must enumerate the REST API host root');
    assert.ok(itemHrefs.includes('https://eagle-eye.app/mcp'), 'item list must enumerate the MCP server');
    assert.ok(
      itemHrefs.includes('https://www.eagle-eye.app/docs/mcp'),
  ...
  it('advertises the docs MCP server on every discovery surface', () => {
    const docsMcpUrl = 'https://www.eagle-eye.app/docs/mcp';
  ...
  it('the docs MCP anchor describes itself with the first-party server-card (service-desc parity with product MCP)', () => {
    const docsAnchor = apiCatalog.linkset.find((e) => e.anchor === 'https://www.eagle-eye.app/docs/mcp');
    ...
    assert.ok(
      desc.some((d) => d.href === 'https://www.eagle-eye.app/.well-known/mcp/docs-server-card.json'),
  ...
  it('the first-party docs server-card advertises the working /docs/mcp endpoint, not the dead mintlify url', () => {
    ...
    // endpoint that actually initializes. eagleeye.mintlify.dev/mcp 404s;
    // www.eagle-eye.app/docs/mcp returns 200. The committed card must carry
    ...
    const WORKING = 'https://www.eagle-eye.app/docs/mcp';
  ...
  it('the api host root has its own context object', () => {
    assert.ok(apiEntry, 'linkset must contain a context object anchored at https://api.eagle-eye.app/');
  });
  ...
  it('status href points at the KEYLESS compact form of /api/health', () => {
    ...
    assert.ok(
      statusHref.startsWith('https://api.eagle-eye.app'),
      `status href must be on api.eagle-eye.app, got: ${statusHref}`
    );
    assert.equal(
      statusHref,
      'https://api.eagle-eye.app/api/health?compact=1',
  ...
  it('service-meta advertises the machine-readable pricing + support surfaces', () => {
    ...
    assert.ok(hrefs.includes('https://eagle-eye.app/pricing.md'), 'service-meta must advertise pricing.md');
    assert.ok(
      hrefs.includes('https://www.eagle-eye.app/api/product-catalog'),
      'service-meta must advertise the live product-catalog JSON endpoint'
    );
    assert.ok(hrefs.includes('https://eagle-eye.app/support.md'), 'service-meta must advertise support.md');
    assert.ok(hrefs.includes('https://eagle-eye.app/agents.md'), 'service-meta must advertise agents.md (#4952)');
    ...
    const commerceSpec = meta.find(
      (entry) => entry.href === 'https://www.eagle-eye.app/docs/openapi/CommerceService.openapi.yaml'
    );
  ...
  it('also advertises a JSON service-desc at /openapi.json for JSON-only parsers', () => {
    ...
    // Read from apiEntry (the api.eagle-eye.app context object), not
  ...
  it('has a second anchor for the MCP server-card', () => {
    const mcpEntry = apiCatalog.linkset.find((entry) => entry.anchor === 'https://eagle-eye.app/mcp');
    assert.ok(mcpEntry, 'linkset must contain an anchor for https://eagle-eye.app/mcp');
  ...
  it('exposes a build:openapi script that copies docs/api → public/openapi.yaml AND emits public/openapi.json', () => {
    ...
    assert.ok(
      buildOpenapi.includes('docs/api/eagleeye.openapi.yaml'),
      `build:openapi must reference docs/api/eagleeye.openapi.yaml, got: ${buildOpenapi}`
    );
    assert.ok(
  ...
  it('openapi source exists at docs/api/eagleeye.openapi.yaml', () => {
    ...
    const openapiPath = resolve(__dirname, '../docs/api/eagleeye.openapi.yaml');
    assert.ok(
      existsSync(openapiPath),
      `docs/api/eagleeye.openapi.yaml must exist — without it, build:openapi fails at deploy time`
    );
  });
});
...
describe('agent readiness: MCP/OAuth origin alignment', () => {
  it('oauth-protected-resource handler returns origin-matching metadata per host', async () => {
    ...
    const hosts = ['eagle-eye.app', 'www.eagle-eye.app', 'api.eagle-eye.app'];
  ...
  it('MCP server card authentication.resource is a valid https URL on a known host', () => {
    ...
    assert.ok(
      ['eagle-eye.app', 'www.eagle-eye.app', 'api.eagle-eye.app'].includes(u.host),
  ...
  it('api/mcp.ts resource_metadata is host-derived, not hardcoded', () => {
    ...
    // api.eagle-eye.app/mcp 401s) and the grep-only test didn't catch it.
    assert.ok(
      !/resource_metadata="https:\/\/(?:api\.)?eagle-eye\.app\/\.well-known\//.test(source),
  ...
  it('discovery handlers reject spoofed Host (apex fallback) and non-GET methods', async () => {
    ...
    for (const host of ['evil.com', 'eagle-eye.app.evil.com', 'evileagle-eye.app', 'x.y.eagle-eye.app']) {
      const prmRes = await prm(new Request('https://eagle-eye.app/.well-known/oauth-protected-resource', { headers: { host } }));
      const prmJson = await prmRes.json();
      assert.equal(prmJson.resource, 'https://eagle-eye.app', `PRM must not reflect spoofed host ${host}`);
      assert.deepEqual(prmJson.authorization_servers, ['https://eagle-eye.app']);

      const asRes = await as(new Request('https://eagle-eye.app/.well-known/oauth-authorization-server', { headers: { host } }));
      const asJson = await asRes.json();
      assert.equal(asJson.issuer, 'https://eagle-eye.app', `AS must not reflect spoofed host ${host}`);
      assert.equal(asJson.token_endpoint, 'https://eagle-eye.app/oauth/token', `AS token_endpoint must not carry spoofed host ${host}`);
      assert.equal(asJson.agent_auth.register_uri, 'https://eagle-eye.app/oauth/register');
      assert.equal(asJson.agent_auth.claim_uri, 'https://eagle-eye.app/oauth/authorize', `AS claim_uri must not carry spoofed host ${host}`);
      assert.equal(asJson.agent_auth.anonymous.claim_uri, 'https://eagle-eye.app/oauth/authorize');
    }

    // Legit subdomain still self-describes.
    const variant = await as(new Request('https://tech.eagle-eye.app/.well-known/oauth-authorization-server', { headers: { host: 'tech.eagle-eye.app' } }));
    assert.equal((await variant.json()).issuer, 'https://tech.eagle-eye.app');

    // Method guard: OPTIONS → 204 preflight, other verbs → 405 + Allow, GET → 200.
    for (const handler of [prm, as]) {
      const opt = await handler(new Request('https://eagle-eye.app/x', { method: 'OPTIONS', headers: { host: 'eagle-eye.app' } }));
      ...
      const post = await handler(new Request('https://eagle-eye.app/x', { method: 'POST', headers: { host: 'eagle-eye.app' } }));
      assert.equal(post.status, 405, 'non-GET/HEAD is rejected');
      assert.equal(post.headers.get('allow'), 'GET, HEAD, OPTIONS');

      const get = await handler(new Request('https://eagle-eye.app/x', { headers: { host: 'eagle-eye.app' } }));
...
describe('agent readiness: auth.md walkthrough', () => {
  ...
  it('advertises a register endpoint that resolves (matches the agent_auth register_uri path)', () => {
    assert.match(
      authMd,
      /https:\/\/(?:api\.)?eagle-eye\.app\/oauth\/register/,
  ...
  it('serves /agent.txt as plain text and keeps it off the SPA catch-all', () => {
    ...
    assert.ok(agentTxt.includes('https://eagle-eye.app/mcp'), 'agent.txt must point at the MCP server');
  });
});
...
describe('agent readiness: homepage Link headers', () => {
  ...
  for (const source of ['/', '/dashboard', '/dashboard.html']) {
    it(`${source} emits a Link header`, () => {
      ...
      // card's url points at eagleeye.mintlify.dev/mcp which 404s on
...
describe('vercel deployment excludes api test files', () => {
  ...
  // EAGLEEYE-VD flooded Sentry with "Upstash Redis is not configured"
...
describe('agent readiness: registry branding + ARD catalog', () => {
  ...
  it('server-card carries the full branding trio and the icon asset exists', () => {
    ...
    assert.match(
      ...
      'server-card icon must be an absolute eagle-eye.app URL'
    );
  ...
  it('ai-catalog.json declares the Eagle Eye host identity', () => {
    assert.strictEqual(aiCatalog.specVersion, '1.0');
    assert.strictEqual(aiCatalog.host?.displayName, 'Eagle Eye');
    assert.strictEqual(aiCatalog.host?.identifier, 'did:web:eagle-eye.app');
  ...
  it('every ai-catalog entry is domain-anchored and complete', () => {
    for (const entry of aiCatalog.entries) {
      ...
      assert.match(
        entry.identifier ?? '',
        /^urn:air:eagle-eye\.app:[a-z-]+:[a-z0-9-]+$/,
...
describe('variant subdomain dashboard SEO (#4996)', () => {
  ...
  const rewriteVariants = dashboardRewrites
    ...
    .map((r) => {
      ...
      return host.replace('.eagle-eye.app', '');
    })
    .sort();

  const middlewareVariants = [...middlewareSource.matchAll(/'([a-z]+)\.eagle-eye\.app': '([a-z]+)'/g)]
    .map((m) => m[2])
    .sort();
  ...
  it('each variant host rewrite targets its generated variant file', () => {
    for (const rule of dashboardRewrites.filter((r) => r.has)) {
      ...
      const variant = host.replace('.eagle-eye.app', '');
      assert.match(host, /^[a-z]+\.eagle-eye\.app$/, `unexpected host condition shape: ${host}`);
      assert.strictEqual(
        rule.destination,
...
describe('docs host scoping — Mintlify proxy is www-only (#5345)', () => {
  // The /docs rewrite proxies eagleeye.mintlify.dev with no host condition,
  ...
  it('redirects subdomain /docs/* to www permanently', () => {
    ...
    assert.equal(docsHostRedirect.destination, 'https://www.eagle-eye.app/docs/:match*');
  ...
  it('the host condition covers every variant subdomain plus api., and never www.', () => {
    ...
    for (const host of [...variantHosts, 'api.eagle-eye.app']) {
      assert.match(host, hostRe, `${host} must be caught by the docs host redirect`);
    }
    assert.ok(!hostRe.test('www.eagle-eye.app'), 'www must keep serving the docs proxy');
  });
  ...
  it('redirects prefix-less /api-reference/* (Googlebot fetches Mintlify RSC route strings) into /docs on www', () => {
    ...
    assert.equal(redirect.destination, 'https://www.eagle-eye.app/docs/api-reference/:match*');
    assert.equal(redirect.permanent, true);
  ...
  it('the www docs proxy rewrite itself is untouched', () => {
    ...
    assert.equal(proxy.destination, 'https://eagleeye.mintlify.dev/docs/:match*');
  });
});
...
describe('markdown canonical Link headers (#4999)', () => {
  ...
  for (const page of MD_PAGES) {
    it(`${page} declares a self-referencing canonical Link header`, () => {
      assert.strictEqual(
        ...
        `<https://www.eagle-eye.app${page}>; rel="canonical"`,
  ...
  it('every sitemap-listed .md URL has the canonical Link header rule', () => {
    const sitemap = readFileSync(resolve(__dirname, '../public/sitemap.xml'), 'utf-8');
    const mdUrls = [...sitemap.matchAll(/<loc>https:\/\/www\.eagle-eye\.app(\/[^<]+\.md)<\/loc>/g)].map((m) => m[1]);
...
// Monitor MCP server", "Eagle Eye OpenAPI", "Eagle Eye developer
// portal", or "Eagle Eye SDK" must land on a crawlable page whose H1 names
...
describe('agent readiness: named developer-resource pages (#4953)', () => {
  const DEV_PAGES = [
    { file: 'developers.md', path: '/developers.md', h1: '# Eagle Eye Developer Portal' },
    { file: 'mcp-server.md', path: '/mcp-server.md', h1: '# Eagle Eye MCP Server' },
    { file: 'openapi.md', path: '/openapi.md', h1: '# Eagle Eye OpenAPI Specification' },
    { file: 'sdks.md', path: '/sdks.md', h1: '# Eagle Eye SDKs' },
  ];
  ...
  it('advertises the developer portal + resource pages across the discovery chain', () => {
    ...
    // The sitemap and the indexed "Build on Eagle Eye" blog post are the two
    ...
    const blogPost = readFileSync(
      resolve(__dirname, '../blog-site/src/content/blog/build-on-eagle-eye-developer-api-open-source.md'),
      'utf-8'
    );
    for (const page of DEV_PAGES) {
      const url = `https://eagle-eye.app${page.path}`;
      ...
      assert.ok(
        sitemap.includes(`https://www.eagle-eye.app${page.path}`),
...
describe('NLWeb schemamap (/schemamap.xml)', () => {
  ...
  it('every advertised <loc> resolves to a tracked file or a live route', () => {
    ...
    const resolvable = {
      ...
      'https://www.eagle-eye.app/blog/rss.xml': () =>
...
describe('docs MCP facade (/docs/mcp)', () => {
  ...
  it('the first-party docs server card still points at the facade URL', () => {
    ...
    assert.equal(card.url, 'https://www.eagle-eye.app/docs/mcp');
    assert.equal(card.serverUrl, 'https://www.eagle-eye.app/docs/mcp');
  });
});
...
describe('section-scoped llms.txt files', () => {
  ...
  it('the site-wide llms.txt cross-links every section file and the sandbox', () => {
    ...
    for (const url of [
      'https://eagle-eye.app/api/llms.txt',
      'https://www.eagle-eye.app/docs/llms.txt',
      'https://eagle-eye.app/developers/llms.txt',
      'https://www.eagle-eye.app/blog/llms.txt',
      'https://www.eagle-eye.app/sandbox/index.json',
      'https://www.eagle-eye.app/schemamap.xml',
    ]) {
...
describe('skeleton brand text extraction (#5541)', () => {
  ...
  it('.skeleton-brand raw textContent does not contain "EEagle"', () => {
    ...
    assert.doesNotMatch(rawText, /EEagle/, 'skeleton-brand raw text must not concatenate as "EEagle Eye"');
    assert.match(rawText, /Eagle Eye/, 'skeleton-brand raw text must contain "Eagle Eye"');
  });
  ...
  it('.skeleton-brand-mark renders "E" via CSS content pseudo-element', () => {
    assert.match(indexHtml, /\.skeleton-brand-mark::after\s*\{\s*content:\s*"E"\s*\}/, 'skeleton-brand-mark must render E via CSS ::after content');
  });
});
