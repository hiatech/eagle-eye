  ...
  it('keeps authenticateInternalRequest fail-closed behavior', async () => {
    ...
    try {
      ...
      const missingSecret = await authenticateInternalRequest(
        new Request('https://eagle-eye.app/api/test', { headers: { Authorization: 'Bearer anything' } }),
      ...
      const wrong = await authenticateInternalRequest(
        new Request('https://eagle-eye.app/api/test', { headers: { Authorization: 'Bearer wrong-secret' } }),
        'WM_TEST_INTERNAL_AUTH_SECRET',
      );
      assert.equal(wrong?.status, 401);

      const ok = await authenticateInternalRequest(
        new Request('https://eagle-eye.app/api/test', { headers: { Authorization: 'Bearer shared-secret' } }),
        'WM_TEST_INTERNAL_AUTH_SECRET',
      );
