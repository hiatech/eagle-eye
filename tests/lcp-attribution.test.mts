...
describe('lcp attribution helpers', () => {
  ...
  it('does not throw when browser storage getters are blocked', () => {
    ...
    Object.defineProperty(globalThis, 'window', {
      configurable: true,
      value: {
        location: { href: 'https://eagle-eye.app/dashboard' },
  ...
  it('redacts query strings from resource URLs', () => {
    assert.equal(
      __testing__.sanitizeResourceUrl('https://api.eagle-eye.app/api/bootstrap?tier=fast&wms=secret#frag'),
      'https://api.eagle-eye.app/api/bootstrap?[redacted]',
    );
    assert.equal(
      __testing__.sanitizeResourceUrl('https://eagle-eye.app/data/countries-110m.json'),
      'https://eagle-eye.app/data/countries-110m.json',
    );
  });
  ...
  it('keeps raw LCP text capture opt-in behind a separate flag', () => {
    ...
    Object.defineProperty(globalThis, 'window', {
      configurable: true,
      value: {
        location: { href: 'https://eagle-eye.app/dashboard?wm_lcp_debug=1' },
    ...
    Object.defineProperty(globalThis, 'window', {
      configurable: true,
      value: {
        location: { href: 'https://eagle-eye.app/dashboard?wm_lcp_text=1' },
        sessionStorage: { getItem: () => null },
        localStorage: { getItem: () => null },
      },
    });
