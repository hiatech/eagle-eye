...
describe('ChokepointInfo proto has ais_disruptions field', () => {
  const proto = readSrc('proto/eagleeye/supply_chain/v1/supply_chain_data.proto');
...
describe('Generated types include aisDisruptions', () => {
  const clientSrc = readSrc('src/generated/client/eagleeye/supply_chain/v1/service_client.ts');
  const serverSrc = readSrc('src/generated/server/eagleeye/supply_chain/v1/service_server.ts');
...
describe('Cache keys bumped to v2', () => {
  const chokepointSrc = readSrc('server/eagleeye/supply-chain/v1/get-chokepoint-status.ts');
  const mineralsSrc = readSrc('server/eagleeye/supply-chain/v1/get-critical-minerals.ts');
...
describe('Chokepoint handler v2 changes', () => {
  const src = readSrc('server/eagleeye/supply-chain/v1/get-chokepoint-status.ts');
...
describe('Minerals handler v2 changes', () => {
  const handlerSrc = readSrc('server/eagleeye/supply-chain/v1/get-critical-minerals.ts');
  const dataSrc = readSrc('server/eagleeye/supply-chain/v1/_minerals-data.ts');
...
describe('Shipping handler v2 changes', () => {
  const src = readSrc('server/eagleeye/supply-chain/v1/get-shipping-rates.ts');
...
import {
  ...
  warningComponent,
  aisComponent,
} from '../server/eagleeye/supply-chain/v1/_scoring.mjs';
...
import { CHOKEPOINTS, THREAT_CONFIG_LAST_REVIEWED } from '../server/eagleeye/supply-chain/v1/get-chokepoint-status.ts';
