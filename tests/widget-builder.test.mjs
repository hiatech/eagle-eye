...
describe('widget-agent relay — security', () => {
  ...
  it('SSRF guard — allowlist is checked before any fetch call in tool loop', () => {
    ...
    // The fetch call to api.eagle-eye.app must come AFTER the check
    const fetchCallIdx = relay.indexOf("'https://api.eagle-eye.app'", allowlistCheck);
    assert.ok(
      fetchCallIdx > allowlistCheck,
      'fetch() to api.eagle-eye.app must appear after allowlist check',
    );
  });
  ...
  it('CORS reuses getCorsOrigin (not a narrow hardcoded origin list)', () => {
    ...
    assert.ok(
      !corsBlock.includes("['https://eagle-eye.app'"),
...
describe('widget-store — constants and logic', () => {
  ...
  it('deleteWidget cleans eagleeye-panel-spans (aggregate map)', () => {
    assert.ok(
  ...
  it('deleteWidget cleans eagleeye-panel-col-spans (aggregate map)', () => {
    assert.ok(
...
describe('proxy routing — widgetAgentUrl', () => {
  ...
  it('widgetAgentUrl targets proxy.eagle-eye.app (not toRuntimeUrl)', () => {
    // The URL may be in a constant above the function; search the whole file
    assert.ok(
      proxy.includes('proxy.eagle-eye.app'),
      'Must target proxy.eagle-eye.app directly (sidecar destroys SSE via arrayBuffer)',
    );
  ...
  it('vite.config.ts proxies /widget-agent to proxy.eagle-eye.app', () => {
    assert.ok(
      vite.includes('/widget-agent'),
      'vite.config.ts must have proxy entry for /widget-agent',
    );
    assert.ok(
      vite.includes('proxy.eagle-eye.app'),
      'Vite proxy target must be proxy.eagle-eye.app',
    );
  });
...
describe('PRO widget — store and sanitizer', () => {
  ...
  it('widget sandbox allows approved Vercel previews and rejects lookalike origins', () => {
    assert.ok(
      sandbox.includes("url.hostname === 'eagle-eye.app'")
        && sandbox.includes("url.hostname.endsWith('.eagle-eye.app')"),
      'sandbox must parse hostname and allow the eagle-eye.app apex/subdomains only',
    );
    assert.ok(
      !sandbox.includes("endsWith('eagle-eye.app')") && !sandbox.includes('endsWith("eagle-eye.app")'),
      'sandbox must not use raw suffix checks that allow evileagle-eye.app',
    );
    ...
    const matchesAllowedTeam = (hostname) =>
      slugs.some((team) =>
        new RegExp('^eagleeye-[a-z0-9-]+-' + team + '\\.vercel\\.app$').test(hostname),
      );
    assert.equal(matchesAllowedTeam('eagleeye-git-feature-eliewm.vercel.app'), true);
    assert.equal(matchesAllowedTeam('eagleeye-abc123-eliewm.vercel.app'), true);
    assert.equal(matchesAllowedTeam('eagleeye-feature-attacker.vercel.app'), false);
    assert.equal(matchesAllowedTeam('eagleeye-git-feature-eliewm.vercel.app.evil.com'), false);
    assert.equal(matchesAllowedTeam('eagleeye-feature-xeliewm.vercel.app'), false);
    assert.equal(matchesAllowedTeam('evileagle-eye.app'), false);
    // The retired personal scope (eagleeye-*-elie-<hash>) must no longer match.
    assert.equal(matchesAllowedTeam('eagleeye-feature-elie-abc123.vercel.app'), false);
    ...
    assert.equal(matchesWithTeammate('eagleeye-feature-kieran.vercel.app'), true);
    assert.equal(matchesWithTeammate('eagleeye-feature-kieranfake.vercel.app'), false);
  });
  ...
  it('widget sandbox behavior accepts Vercel previews and blocks spoofed parents', () => {
    ...
    const allowed = runSandbox('https://eagleeye-git-feature-eliewm.vercel.app/dashboard');
    assert.equal(allowed.readyMessages.length, 1);
    assert.equal(allowed.readyMessages[0].targetOrigin, 'https://eagleeye-git-feature-eliewm.vercel.app');
    ...
    allowed.message({
      ...
      origin: 'https://eagleeye-git-feature-eliewm.vercel.app',
    ...
    for (const invalidMessage of invalidMessages) {
      const rejected = runSandbox('https://eagle-eye.app/dashboard');
      rejected.message({ ...invalidMessage, source: rejected.parent });
      assert.deepEqual(rejected.writes, []);
    }

    const wrongSource = runSandbox('https://eagle-eye.app/dashboard');
    wrongSource.message({
    ...
    const spoofed = runSandbox('https://eagleeye-git-feature-eliewm.vercel.app.evil.com/');
    assert.deepEqual(spoofed.readyMessages, []);
    spoofed.message({
      data: { type: 'wm-html', id: 'wm-1', token: 'test-token', html: '<p>bad</p>' },
      origin: 'https://eagleeye-git-feature-eliewm.vercel.app.evil.com',
  ...
  it('widget sandbox blocks beforeunload without breaking normal events before widget execution', () => {
    ...
    let vmContext;
    const document = {
      referrer: 'https://eagle-eye.app/dashboard',
      open() {
