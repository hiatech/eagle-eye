...
// "eagleeye" (confirmed live -- see the HAPI_APP_IDENTIFIER comment in
...
// field back toward "eagleeye..." would silently reintroduce the exact
...
// is deliberately left containing "eagleeye" as the real contact address,
...
for (const relPath of ['shared/hapi-app-identifier.json', 'scripts/shared/hapi-app-identifier.json']) {
  test(`${relPath}: application field does not contain "eagleeye"`, () => {
    const cfg = loadConfig(relPath);
    assert.equal(typeof cfg.application, 'string');
    assert.doesNotMatch(
      cfg.application.toLowerCase(), /eagleeye/,
      `${relPath}'s application field ("${cfg.application}") contains "eagleeye" -- ` +
