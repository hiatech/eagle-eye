...
describe('api/mcp.ts — PRO MCP Server', () => {
  ...
  it('returns HTTP 401 + WWW-Authenticate when no credentials provided (protected method)', async () => {
    ...
    assert.ok(res.headers.get('www-authenticate')?.includes('Bearer realm="eagleeye"'), 'must include WWW-Authenticate header');
  ...
  it('initialize succeeds WITHOUT credentials (public discovery)', async () => {
    ...
    assert.equal(body.result?.serverInfo?.name, 'eagleeye');
  ...
  it('resources/read of a PUBLIC resource succeeds WITHOUT credentials + never touches quota (orank mcp-resource-quality)', async () => {
    ...
    const req = new Request(BASE_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ jsonrpc: '2.0', id: 6, method: 'resources/read', params: { uri: 'eagleeye://seed-meta/freshness' } }),
    });
    const res = await handler(req);
  ...
  it('resources/read of a data-bearing TEMPLATE instantiation still requires credentials (no quota bypass)', async () => {
    const req = new Request(BASE_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ jsonrpc: '2.0', id: 7, method: 'resources/read', params: { uri: 'eagleeye://countries/de/risk' } }),
    });
    const res = await handler(req);
  ...
  it('returns HTTP 401 + WWW-Authenticate when invalid API key provided', async () => {
    const req = makeReq('POST', initBody(), { 'X-EagleEye-Key': 'wrong_key' });
    const res = await handler(req);
    assert.equal(res.status, 401);
    const wwwAuth = res.headers.get('www-authenticate') ?? '';
    assert.ok(wwwAuth.includes('Bearer realm="eagleeye"'), 'must include WWW-Authenticate Bearer realm');
  ...
  it('OPTIONS returns 204 with CORS headers', async () => {
    const req = new Request(BASE_URL, { method: 'OPTIONS', headers: { origin: 'https://eagle-eye.app' } });
    const res = await handler(req);
  ...
  it('malformed body returns JSON-RPC -32600', async () => {
    const req = new Request(BASE_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-EagleEye-Key': VALID_KEY },
      body: '{bad json',
    });
    const res = await handler(req);
    const body = await res.json();
  ...
  it('get_maritime_activity returns zones and disruptions for valid country code', async () => {
    ...
    // in production — EAGLEEYE-T8. Items outside the AE bbox (+3° pad)
...
describe('api/mcp.ts — U7 Pro-path', () => {
  ...
  it('edge: Pro tool _execute fetch sends X-WM-MCP-Internal + X-WM-MCP-User-Id, no X-EagleEye-Key', async () => {
    const { deps } = makeProDeps();
    let captured = null;
    globalThis.fetch = async (url, init) => {
    ...
    assert.equal(captured.headers.get('x-eagleeye-key'), null, 'X-EagleEye-Key must NOT be set for Pro');
  ...
  it('edge: Pro get_country_brief signs a short URL and sends grounding context in the POST body', async () => {
    ...
    assert.equal(countryCall.headers.get('x-eagleeye-key'), null, 'X-EagleEye-Key must NOT be set for Pro');
  ...
  it('integration: signed header for /api/news/v1/list-feed-digest cannot be replayed against /api/intelligence/v1/deduct-situation', async () => {
    ...
    const signed = await signInternalMcpRequest({
      method: 'GET',
      url: 'https://eagle-eye.app/api/news/v1/list-feed-digest?lang=en&variant=full',
      body: null,
      userId: PRO_USER_ID,
      secret: HMAC_SECRET,
    });
    // Re-construct the payload that would be expected for the SAME ts on a different path.
    const replayUrl = new URL('https://eagle-eye.app/api/intelligence/v1/deduct-situation');
