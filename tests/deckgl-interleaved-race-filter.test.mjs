...
describe('DeckGLMap interleaved render race console filter', () => {
  it('matches the deck.gl null-id race when Sentry owns ev.filename but deck-stack is in the stack', () => {
    const stack = [
      ...
      '    at DeckRenderer._drawLayers (https://app.eagle-eye.app/assets/deck-stack-Dq2qX5Bt.js:1606:18)',
      '    at LayerManager.renderLayers (https://app.eagle-eye.app/assets/deck-stack-Dq2qX5Bt.js:5000:12)',
      '    at r (https://app.eagle-eye.app/assets/sentry-DMxp_zBn.js:488:7)',
    ].join('\n');

    assert.equal(shouldSuppress({
      message: "Cannot read properties of null (reading 'id')",
      filename: 'https://app.eagle-eye.app/assets/sentry-DMxp_zBn.js',
      stack,
    }), true);
  });
  ...
  it('still matches direct deck-stack filenames, including bare chunk names', () => {
    ...
    assert.equal(shouldSuppress({
      message: "Cannot read properties of null (reading 'id')",
      filename: 'https://app.eagle-eye.app/assets/deck-stack-Dq2qX5Bt.js',
    }), true);
  });
  ...
  it('does not suppress the same null-id message without deck-stack evidence', () => {
    assert.equal(shouldSuppress({
      message: "Cannot read properties of null (reading 'id')",
      filename: 'https://app.eagle-eye.app/assets/main-Dq2qX5Bt.js',
      stack: '    at firstPartyRender (https://app.eagle-eye.app/assets/main-Dq2qX5Bt.js:10:2)',
    }), false);
  });
