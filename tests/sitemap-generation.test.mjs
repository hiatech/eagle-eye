...
describe('root sitemap generator', () => {
  it('declares owned material-content sources without copying blog or docs inventories', () => {
    ...
    assert.ok(locations.includes('https://eagle-eye.app/mcp'));
    assert.ok(locations.includes(`${SITE_ORIGIN}/pricing.md`));
    assert.ok(locations.includes('https://tech.eagle-eye.app/dashboard'));
  ...
  it('generates byte-identical canonical entries from static owners and corpus page metadata', () => {
    ...
    try {
      ...
      assert.match(first, /<loc>https:\/\/www\.eagle-eye\.app\/countries\/norway\/<\/loc>/);
      assert.match(first, /<loc>https:\/\/www\.eagle-eye\.app\/tools\/natural-hazard-pulse\/<\/loc>/);
      ...
      assert.doesNotMatch(first, /eagle-eye\.app\/blog/);
      assert.doesNotMatch(first, /eagle-eye\.app\/docs/);
  ...
  it('rejects malformed, noncanonical, duplicate, missing, and future metadata', () => {
    ...
    assert.throws(
      () => validateSitemapEntries([{ ...valid, loc: 'http://www.eagle-eye.app/dashboard' }], { today: '2026-07-27' }),
      /https/i,
    );
    assert.throws(
  ...
  it('preserves committed freshness when Git history is shallow', () => {
    ...
    try {
      ...
      execFileSync('git', ['config', 'user.email', 'sitemap-test@eagle-eye.app'], {
        cwd: sourceRoot,
        env: gitEnv,
      });
  ...
  it('keeps the committed artifact generated and robots ownership exact', () => {
    ...
    assert.doesNotMatch(sitemap, /eagle-eye\.app\/blog/);
    assert.match(sitemap, /<loc>https:\/\/eagle-eye\.app\/mcp<\/loc>/);
