...
describe('readQueryLanguage — wmQuery detector lookup', () => {
  it('returns the lang param for shareable application-language state', () => {
    assert.equal(readQueryLanguage('https://www.eagle-eye.app/dashboard?lang=fa'), 'fa');
    assert.equal(readQueryLanguage('https://www.eagle-eye.app/dashboard?foo=1&lang=hr'), 'hr');
  });

  it('returns undefined (fall through to next detector) when lang is absent or empty', () => {
    assert.equal(readQueryLanguage('https://www.eagle-eye.app/dashboard'), undefined);
    assert.equal(readQueryLanguage('https://www.eagle-eye.app/dashboard?lang='), undefined);
    assert.equal(readQueryLanguage('https://www.eagle-eye.app/dashboard?other=x'), undefined);
  });
...
describe('stripQueryLanguage — explicit-choice reload guard', () => {
  it('removes the lang param so a stale ?lang does not out-rank the saved choice on reload', () => {
    assert.equal(
      stripQueryLanguage('https://www.eagle-eye.app/dashboard?lang=fa'),
  ...
  it('preserves every other query param and the hash', () => {
    assert.equal(
      stripQueryLanguage('https://www.eagle-eye.app/dashboard?view=map&lang=de&zoom=3#panel'),
      'https://www.eagle-eye.app/dashboard?view=map&zoom=3#panel',
    );
  });

  it('returns the URL unchanged when there is no lang param', () => {
    const href = 'https://www.eagle-eye.app/dashboard?view=map';
