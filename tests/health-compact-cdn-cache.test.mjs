...
test('compact 200 is never edge-cached (monitors must see live recovery)', async () => {
  mockRedisPipeline();
  const res = await handler(new Request('https://api.eagle-eye.app/api/health?compact=1'));
...
test('detailed (key-authenticated) 200 stays no-store', async () => {
  mockRedisPipeline();
  const res = await handler(new Request('https://api.eagle-eye.app/api/health', {
