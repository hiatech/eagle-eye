...
function call(pathOrUrl: string, ua: string, headers: Record<string, string> = {}): Response | void {
  const url = pathOrUrl.startsWith('http')
    ? pathOrUrl
    : `https://www.eagle-eye.app${pathOrUrl}`;
...
describe('middleware bot gate / keyed API clients', () => {
  ...
  it('passes a 40-hex user API key through when curl would otherwise be blocked', () => {
    const res = call(KEYED_API_PATH, GENERIC_CURL_UA, { 'x-eagleeye-key': USER_API_KEY });
  ...
  it('passes a 48-hex enterprise API key through when curl would otherwise be blocked', () => {
    const res = call(KEYED_API_PATH, GENERIC_CURL_UA, { 'x-eagleeye-key': ENTERPRISE_API_KEY });
  ...
  it('still blocks malformed and overlong wm_ keys with a curl UA', () => {
    for (const apiKey of [`wm_${'c'.repeat(39)}`, `wm_${'d'.repeat(65)}`, 'wm_not-hex']) {
      const res = call(KEYED_API_PATH, GENERIC_CURL_UA, { 'x-eagleeye-key': apiKey });
...
// The MCP endpoint's canonical URL is apex (`https://eagle-eye.app/mcp`).
...
describe('middleware /mcp — variant subdomains redirect to apex, POST stays', () => {
  it('redirects GET /mcp from tech.eagle-eye.app to apex', () => {
    const res = call('https://tech.eagle-eye.app/mcp', CHROME_UA);
    assert.ok(res instanceof Response);
    assert.equal(res.status, 308);
    assert.equal(res.headers.get('location'), 'https://eagle-eye.app/mcp');
  });

  it('redirects HEAD /mcp from finance.eagle-eye.app to apex', () => {
    const req = new Request('https://finance.eagle-eye.app/mcp', { method: 'HEAD' });
  ...
  it('redirects /mcp from every variant subdomain', () => {
    for (const host of ['tech', 'finance', 'commodity', 'happy', 'energy']) {
      const res = call(`https://${host}.eagle-eye.app/mcp`, CHROME_UA);
      assert.ok(res instanceof Response, `${host} must redirect`);
      assert.equal(res.status, 308, `${host} redirect status`);
      assert.equal(res.headers.get('location'), 'https://eagle-eye.app/mcp', `${host} redirect location`);
    }
  });

  it('does NOT redirect GET /mcp from apex or www', () => {
    assert.equal(call('https://eagle-eye.app/mcp', CHROME_UA), undefined);
    assert.equal(call('https://www.eagle-eye.app/mcp', CHROME_UA), undefined);
  });

  it('does NOT redirect POST /mcp from a variant subdomain (MCP handshake)', () => {
    const req = new Request('https://tech.eagle-eye.app/mcp', {
      method: 'POST',
  ...
  it('does NOT redirect variant transport GETs with SSE or replay headers', () => {
    const mixedCaseSse = new Request('https://tech.eagle-eye.app/mcp', {
      headers: { Accept: 'Text/Event-Stream' },
    });
    assert.equal(middleware(mixedCaseSse), undefined, 'mixed-case SSE Accept must fall through to the transport');

    const replay = new Request('https://tech.eagle-eye.app/mcp', {
