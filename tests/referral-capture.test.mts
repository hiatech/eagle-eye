...
before(() => {
  ...
  _loc = { href: 'https://eagle-eye.app/', pathname: '/', search: '', hash: '' };
...
describe('captureReferralFromUrl', () => {
  it('captures ?ref= into localStorage and strips from URL', () => {
    setUrl('https://eagle-eye.app/?ref=abc123');
    const captured = captureReferralFromUrl();
    assert.equal(captured, 'abc123');
    assert.equal(_loc.href, 'https://eagle-eye.app/');
  ...
  it('captures ?wm_referral= (dashboard-forward param name)', () => {
    setUrl('https://eagle-eye.app/?wm_referral=xyz789');
    const captured = captureReferralFromUrl();
  ...
  it('prefers wm_referral over ref when both are present', () => {
    setUrl('https://eagle-eye.app/?ref=old&wm_referral=new');
    const captured = captureReferralFromUrl();
  ...
  it('returns null when no referral param is present', () => {
    setUrl('https://eagle-eye.app/?other=value');
    assert.equal(captureReferralFromUrl(), null);
    assert.equal(_loc.href, 'https://eagle-eye.app/?other=value');
    assert.equal(_localStorage.getItem(REFERRAL_CAPTURE_KEY), null);
  });

  it('preserves non-referral query params when stripping', () => {
    setUrl('https://eagle-eye.app/?ref=abc&topic=brief');
    captureReferralFromUrl();
    assert.equal(_loc.href, 'https://eagle-eye.app/?topic=brief');
  });

  it('rejects invalid codes (whitespace, special chars) without crashing', () => {
    setUrl('https://eagle-eye.app/?ref=' + encodeURIComponent('<script>alert(1)</script>'));
    const captured = captureReferralFromUrl();
  ...
  it('rejects excessively long codes', () => {
    ...
    setUrl(`https://eagle-eye.app/?ref=${huge}`);
    assert.equal(captureReferralFromUrl(), null);
  });

  it('accepts underscore and hyphen in codes', () => {
    setUrl('https://eagle-eye.app/?ref=some_code-v2');
...
describe('appendRefToUrl', () => {
  it('appends wm_referral to a bare URL', () => {
    assert.equal(
      appendRefToUrl('https://eagle-eye.app', 'abc'),
      'https://eagle-eye.app/?wm_referral=abc',
    );
  });

  it('preserves existing query params', () => {
    assert.equal(
      appendRefToUrl('https://eagle-eye.app/?topic=brief', 'abc'),
      'https://eagle-eye.app/?topic=brief&wm_referral=abc',
    );
  });

  it('returns input unchanged when refCode is falsy', () => {
    assert.equal(appendRefToUrl('https://eagle-eye.app', undefined), 'https://eagle-eye.app');
    assert.equal(appendRefToUrl('https://eagle-eye.app', null), 'https://eagle-eye.app');
    assert.equal(appendRefToUrl('https://eagle-eye.app', ''), 'https://eagle-eye.app');
  });

  it('returns input unchanged for invalid codes', () => {
    assert.equal(
      appendRefToUrl('https://eagle-eye.app', 'bad code with spaces'),
      'https://eagle-eye.app',
    );
  });
...
describe('round-trip: capture → load → clear', () => {
  it('captures from /pro?ref=, loads on dashboard, clears after attribution', () => {
    // 1. /pro with ref
    setUrl('https://eagle-eye.app/pro?ref=sharerA');
    captureReferralFromUrl();
  ...
  it('second capture in same session replaces prior code (new share link wins)', () => {
    setUrl('https://eagle-eye.app/?ref=first');
    captureReferralFromUrl();
    assert.equal(loadActiveReferral(), 'first');

    setUrl('https://eagle-eye.app/?ref=second');
    captureReferralFromUrl();
