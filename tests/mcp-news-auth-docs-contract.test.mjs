...
async function captureRpcFetches(toolName, params, opts = {}) {
  ...
  const result = await rpcTool(toolName)._execute(params, 'https://eagle-eye.app', {
    kind: 'env_key',
...
describe('MCP news/auth public contract', () => {
  ...
  it('MCP-facing docs and fixture helpers do not teach stale API-key prefixes', () => {
    ...
    assert.match(quickstart, /X-EagleEye-Key/, 'quickstart must teach API keys via X-EagleEye-Key');
