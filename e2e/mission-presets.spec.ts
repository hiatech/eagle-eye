...
const PRESET_KEY = 'eagleeye-mission-preset-v1';
...
test.describe('mission presets', () => {
  test('desktop first-run mission can apply and persist across reload', async ({ page }) => {
    ...
    await expect
      ...
      .toBe('supply-chain');
    await expect
      .poll(() => readJsonLocalStorage<Record<string, boolean>>(page, 'eagleeye-layers').then((layers) => layers?.tradeRoutes))
      .toBe(true);
  ...
  test('mobile mission picker stays in viewport and applies from the mobile menu', async ({ page }) => {
    ...
    await expect
      .poll(() => readJsonLocalStorage<Record<string, boolean>>(page, 'eagleeye-layers').then((layers) => layers?.pipelines ?? false))
      .toBe(true);
    await expect
