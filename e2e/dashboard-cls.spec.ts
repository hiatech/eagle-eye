...
const installDashboardClsObserver = async (
  ...
  await page.addInitScript(({ dismissKey, legacyDismissKey, seedDeferredFootprint, panelId, panelOrder }) => {
    ...
    if (seedDeferredFootprint) {
      ...
      localStorage.setItem('eagleeye-panel-spans', JSON.stringify({ [panelId]: 3 }));
      localStorage.setItem('eagleeye-panel-col-spans', JSON.stringify({ [panelId]: 2 }));
      localStorage.removeItem('eagleeye-panel-collapsed');
    }
