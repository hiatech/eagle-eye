...
test.describe('theme toggle (happy variant)', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.addInitScript(() => {
      if (!sessionStorage.getItem('__test_init_done')) {
        localStorage.removeItem('eagleeye-theme');
        localStorage.removeItem('eagleeye-variant');
  ...
  test('dark mode persists across page reload', async ({ page }) => {
    ...
    const stored = await page.evaluate(() => localStorage.getItem('eagleeye-theme'));
  ...
  test('no FOUC: data-theme is set before main CSS loads', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      localStorage.setItem('eagleeye-theme', 'dark');
      localStorage.setItem('eagleeye-variant', 'happy');
    });
