...
function renderOverview(area: HTMLElement): void {
  ...
  const wmState = getSecretState('EAGLEEYE_API_KEY');
  ...
  setTrustedHtml(area, trustedHtml(`
    ...
    <div class="settings-ov-license">
      <section class="wm-section">
        <h2 class="wm-section-title">${t('modals.settingsWindow.eagleEye.apiKey.title')}</h2>
        <p class="wm-section-desc">${t('modals.settingsWindow.eagleEye.apiKey.description')}</p>
        <div class="wm-key-row">
          <div class="wm-input-wrap">
            <input type="password" class="wm-input" data-wm-key-input
              placeholder="${t('modals.settingsWindow.eagleEye.apiKey.placeholder')}"
      ...
      <div class="wm-divider"><span>${t('modals.settingsWindow.eagleEye.dividerOr')}</span></div>

      <section class="wm-section">
        <h2 class="wm-section-title">${t('modals.settingsWindow.eagleEye.register.title')}</h2>
        <p class="wm-section-desc">${t('modals.settingsWindow.eagleEye.register.description')}</p>
        <div class="wm-register-row">
          <button type="button" class="wm-submit-btn" data-wm-open-pro>
            ${t('modals.settingsWindow.eagleEye.register.submitBtn')}
          </button>
        </div>
      </section>
    </div>
...
function initOverviewListeners(area: HTMLElement): void {
  ...
  area.querySelector('[data-wm-open-pro]')?.addEventListener('click', () => {
    const url = 'https://eagle-eye.app/pro';
    ...
    void (async () => {
      try {
        ...
        if (hasWmKeyChange && wmKeyValue) {
          await setSecretValue('EAGLEEYE_API_KEY', wmKeyValue);
        }
