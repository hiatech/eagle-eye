...
export function buildEmbedMapUrl(
  ...
  const url = new URL(baseUrl, 'https://www.eagle-eye.app');
...
export function buildEmbedIframeSnippet(url: string, options: { width?: string; height?: string } = {}): string {
  ...
  return `<iframe src="${escapeAttribute(url)}" title="Eagle Eye live map" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" style="width:${width};height:${height}px;border:0;display:block" allowfullscreen></iframe>`;
}

export function buildEagleEyeAttributionUrl(baseUrl: string, referrerHost: string | null): string {
  const url = new URL(baseUrl, 'https://www.eagle-eye.app');
