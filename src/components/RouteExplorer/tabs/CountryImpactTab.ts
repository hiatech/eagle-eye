...
import type {
  ...
  StrategicProduct,
} from '@/generated/server/eagleeye/supply_chain/v1/service_server';
import {
...
export class CountryImpactTab {
  ...
  private renderMissing(): void {
    setTrustedHtml(this.element, trustedHtml('<div class="re-tab__empty">' +
      ...
      '<p>EagleEye does not have bilateral trade data for this destination country yet.</p>' +
  ...
  private renderLazy(): void {
    setTrustedHtml(this.element, trustedHtml('<div class="re-tab__empty">' +
      ...
      '<p>EagleEye is fetching trade data for this destination for the first time. ' +
  ...
  private renderData(data: GetRouteImpactResponse): void {
    const bannerHtml = !data.hs2InSeededUniverse
      ? '<div class="re-impact__banner">Lane value for this HS code is not in EagleEye\'s strategic-products dataset. Top strategic products shown below.</div>'
      : '';
