...
function downloadStory(): void {
  ...
  a.download = `eagleeye-${currentData?.countryCode.toLowerCase() || 'story'}-${Date.now()}.png`;
  a.click();
...
async function shareWhatsApp(data: StoryData): Promise<void> {
  ...
  const file = new File([currentBlob], `${data.countryCode.toLowerCase()}-eagleeye.png`, { type: 'image/png' });
