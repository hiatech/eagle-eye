/**
 * LatestBriefPanel — dashboard surface for the EagleEye Brief.
 *
...
const WM_LOGO_SVG: TrustedHtml = trustedHtml(
  ...
  'Static EagleEye logo SVG defined in source',
);
...
export class LatestBriefPanel extends Panel {
  ...
  private async fetchLatest(signal: AbortSignal): Promise<LatestBriefResponse> {
    ...
    // short-circuits on desktop EAGLEEYE_API_KEY / tester keys
    ...
    this.content.appendChild(
      h('div', { className: 'latest-brief-card latest-brief-card--composing' },
        ...
        h('div', { className: 'latest-brief-empty-body' },
          'Your personalised brief is tied to your EagleEye account. Sign in to see today\u2019s issue.',
        ),
      ),
    );
  }
  ...
  private renderUpgradeRequired(): void {
    ...
    this.content.appendChild(
      h('div', { className: 'latest-brief-card latest-brief-card--composing' },
        ...
        h('div', { className: 'latest-brief-empty-body' },
          'The EagleEye Brief is included with the Pro plan. Upgrade to unlock today\u2019s issue.',
        ),
      ),
    );
  }
  ...
  private renderComposing(data: LatestBriefComposing): void {
    ...
    this.content.appendChild(
      h('div', { className: 'latest-brief-card latest-brief-card--composing' },
        ...
        h('div', { className: 'latest-brief-empty-body' },
          `The editorial team at EagleEye is writing your ${data.issueDate} brief. Check back in a moment.`,
        ),
      ),
    );
  }
  ...
  private renderReady(data: LatestBriefReady): void {
    ...
    const coverCard = h('a', {
      ...
      h('div', { className: 'latest-brief-cover' },
        ...
        h('div', { className: 'latest-brief-cover-title' }, 'EagleEye'),
    ...
    const shareBtn = h('button', {
      ...
      'aria-label': 'Share EagleEye — copies a referral link',
      disabled: true,
    }, 'Share ↗');
