...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!proAccess.allowed) {
    ...
    return new Response(JSON.stringify({ error: 'pro_required', message: 'Slack notifications are available on the Pro plan.', upgradeUrl: 'https://eagle-eye.app/pro' }), { status: 403, headers: { 'Content-Type': 'application/json', ...corsHeaders } });
  }
