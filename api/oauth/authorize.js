...
// First-party origin allowlist for the consent POST: the eagle-eye.app apex
...
// api.eagle-eye.app) and its JS same-origin fetch both arrive with a
// first-party Origin — which, before this, only matched api.eagle-eye.app and
...
// eagle-eye.app.evil.example, evileagle-eye.app, and any :port.
const WM_ORIGIN = /^https:\/\/(?:[a-z0-9-]+\.)?eagle-eye\.app$/;
...
function htmlError(title, detail) {
  return new Response(`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Error &#x2014; EagleEye MCP</title>
...
<body><a href="https://www.eagle-eye.app" class="wm-logo" target="_blank" rel="noopener">${GLOBE_SVG}<span class="wm-logo-text">EagleEye MCP</span></a>
<div class="card"><h1>${escapeHtml(title)}</h1><p>${escapeHtml(detail)}</p><a href="javascript:history.back()" class="back">&#8592; go back</a></div>
<p class="footer"><a href="https://www.eagle-eye.app" target="_blank" rel="noopener">eagle-eye.app</a></p>
...
export function consentPage(params, nonce, errorMsg = '') {
  ...
  const proCtaHref = `https://eagle-eye.app/mcp-grant?nonce=${encodeURIComponent(nonce)}`;
  return new Response(`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Authorize &#x2014; EagleEye MCP</title>
<style>
...
<body>
<a href="https://www.eagle-eye.app" class="wm-logo" target="_blank" rel="noopener">${GLOBE_SVG}<span class="wm-logo-text">EagleEye MCP</span></a>
<div class="card">
<div class="client-hd">
...
<hr>
<a id="pc" class="pro-cta" href="${escapeHtml(proCtaHref)}">Sign in with EagleEye Pro</a>
<div class="disclosure" id="dt"><a id="tk" role="button" tabindex="0">Use API key instead</a></div>
<form id="cf" method="POST" action="https://api.eagle-eye.app/oauth/authorize" style="display:none;margin-top:1.25rem">
...
<p class="hint">No key? <a href="https://www.eagle-eye.app/pro" target="_blank" rel="noopener">Get one at eagle-eye.app/pro &#x2192;</a></p>
<p class="error" id="ke"${errorMsg ? '' : ' style="display:none"'}>${errorMsg ? escapeHtml(errorMsg) : ''}</p>
<button type="submit" id="ab">Authorize</button>
</form>
</div>
<p class="footer"><a href="https://www.eagle-eye.app" target="_blank" rel="noopener">eagle-eye.app</a> &middot; <a href="https://www.eagle-eye.app/pro" target="_blank" rel="noopener">Get an API key &#x2192;</a></p>
...
export default async function handler(req) {
  ...
  if (method === 'POST') {
    // Origin validation: allow any first-party eagle-eye.app host (the consent
    // page is served host-derived — apex/www/api/variant — so a same-origin JS
    // fetch or the native form POST to api.eagle-eye.app arrives with any of
