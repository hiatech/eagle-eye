...
function makePreflight(origin) {
  return new Request('https://api.eagle-eye.app/api/bootstrap?keys=techReadiness', {
    method: 'OPTIONS',
    headers: {
      origin,
...
test('bootstrap preflight is compatible with credentialed browser fetches', async () => {
  const resp = await handler(makePreflight('https://www.eagle-eye.app'));

  assert.equal(resp.status, 204);
  assert.equal(resp.headers.get('access-control-allow-origin'), 'https://www.eagle-eye.app');
...
test('bootstrap GET response is compatible with credentialed browser fetches', async () => {
  ...
  try {
    ...
    const resp = await handler(new Request('https://api.eagle-eye.app/api/bootstrap?keys=techReadiness', {
      method: 'GET',
      headers: {
