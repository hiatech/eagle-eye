...
function makeRequest(query = '') {
  return new Request(`https://eagle-eye.app/api/youtube/embed${query}`);
}
...
test('returns embeddable html for valid video id', async () => {
  ...
  assert.equal(html.includes('origin:"https://eagle-eye.app"'), true);
...
test('uses dedicated parentOrigin for iframe postMessage target', async () => {
  const response = await handler(makeRequest('?videoId=iEpJwprxDdk&origin=https://eagle-eye.app&parentOrigin=https://tauri.localhost'));
  const html = await response.text();
  assert.match(html, /playerVars:\{[^}]*origin:"https:\/\/eagle-eye\.app"/);
...
test('does not accept wildcard parentOrigin query parameter', async () => {
  const response = await handler(makeRequest('?videoId=iEpJwprxDdk&origin=https://eagle-eye.app&parentOrigin=*'));
  const html = await response.text();
  assert.equal(html.includes('parentOrigin="*"'), false);
  assert.match(html, /parentOrigin="https:\/\/eagle-eye\.app"/);
});
