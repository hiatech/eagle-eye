...
# Official Ruby SDK for the Eagle Eye global-intelligence API.
#
# Stdlib-only (Net::HTTP), MCP-first — the same design as the `eagleeye`
# npm CLI this mirrors (`cli/` in the main repository). The MCP server
# (https://eagle-eye.app/mcp) is the live, documented agent surface:
...
module EagleEye
  ...
  AUTH_HINT = "Hint: this call needs a key - pass api_key: or set " \
              "EAGLEEYE_API_KEY (get one at https://eagle-eye.app/pro)."
  ...
  # EAGLEEYE_API_KEY (or WM_API_KEY), EAGLEEYE_BASE_URL, and
  # EAGLEEYE_MCP_URL environment variables, then to the public production
  ...
  class Client
    ...
    def initialize(api_key: nil, base_url: nil, mcp_url: nil,
      ...
      @api_key = api_key || env["EAGLEEYE_API_KEY"] || env["WM_API_KEY"]
      @base_url = (base_url || env["EAGLEEYE_BASE_URL"] || DEFAULT_BASE_URL).sub(%r{/+\z}, "")
      @mcp_url = mcp_url || env["EAGLEEYE_MCP_URL"] || DEFAULT_MCP_URL
      @timeout = timeout
