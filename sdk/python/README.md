...
Official Python SDK for the [Eagle Eye](https://eagle-eye.app) global-intelligence API — country briefs, risk scores, conflict / cyber / market / news feeds, and every MCP tool, without writing an HTTP integration.
...
- `eagleeye_sdk.MCPError` — the MCP server returned a JSON-RPC error (`.code`, auth failures carry a key hint).
- `eagleeye_sdk.APIError` — a REST/transport failure (`.status`, `.body`).

Both derive from `eagleeye_sdk.EagleEyeError`.
...
The source lives in [`sdk/python/`](https://github.com/hiatech/eagle-eye/tree/main/sdk/python) in the main repository. Docs: [eagle-eye.app/docs/sdks](https://www.eagle-eye.app/docs/sdks). License: MIT (thin client; the Eagle Eye platform itself remains AGPL-3.0).
