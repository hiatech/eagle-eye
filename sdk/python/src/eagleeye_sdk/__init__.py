"""Official Python SDK for the Eagle Eye global-intelligence API.

Dependency-free (stdlib only), MCP-first — the same design as the
``eagleeye`` npm CLI this mirrors (``cli/`` in the main repository).
The MCP server (https://eagle-eye.app/mcp) is the live, documented agent
...
(``get``/``health``) rounds it out for host-relative and self-hosted use.
    ...
    client = Client(api_key="wm_...")       # or env EAGLEEYE_API_KEY
...
USER_AGENT = "eagleeye-python/%s (+https://eagle-eye.app)" % __version__

DEFAULT_BASE_URL = "https://api.eagle-eye.app"
DEFAULT_MCP_URL = "https://eagle-eye.app/mcp"

# Header the API accepts for a user-issued key (alias: X-Api-Key).
_AUTH_HEADER_NAME_PARTS = ("X", "EagleEye", "Key")
...
AUTH_HINT = (
    "Hint: this call needs a key - pass api_key= or set EAGLEEYE_API_KEY "
    "(get one at https://eagle-eye.app/pro)."
)
...
class Client:
    """Thin client for the Eagle Eye MCP server and REST API.

    All keyword arguments are optional; unset values fall back to the
    ``EAGLEEYE_API_KEY`` (or ``WM_API_KEY``), ``EAGLEEYE_BASE_URL``,
    and ``EAGLEEYE_MCP_URL`` environment variables, then to the public
    ...
    def __init__(
        ...
        self.api_key = api_key or env.get("EAGLEEYE_API_KEY") or env.get("WM_API_KEY")
        self.base_url = (base_url or env.get("EAGLEEYE_BASE_URL") or DEFAULT_BASE_URL).rstrip("/")
        self.mcp_url = mcp_url or env.get("EAGLEEYE_MCP_URL") or DEFAULT_MCP_URL
