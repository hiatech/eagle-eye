// Offline tests for the eagleeye Go SDK (httptest server, no network).
package eagleeye
...
func TestMCPErrorWinsOverHTTP200(t *testing.T) {
	...
	if !strings.Contains(mcpErr.Error(), "EAGLEEYE_API_KEY") {
...
func TestNon2xxReturnsAPIError(t *testing.T) {
	...
	if !strings.Contains(apiErr.Error(), "EAGLEEYE_API_KEY") {
		t.Fatalf("auth hint missing: %s", apiErr.Error())
	}
}

func TestUserAgentCarriesVersion(t *testing.T) {
	if !strings.HasPrefix(UserAgent, "eagleeye-go/") || !strings.Contains(UserAgent, Version) {
		t.Fatalf("UserAgent = %q", UserAgent)
	}
	if !strings.Contains(UserAgent, "+https://eagle-eye.app") {
