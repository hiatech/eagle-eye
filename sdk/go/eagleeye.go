// Package eagleeye is the official Go SDK for the Eagle Eye
// global-intelligence API (https://eagle-eye.app) — country briefs, risk
...
// eagleeye npm CLI this mirrors (cli/ in the main repository). The MCP
// server (https://eagle-eye.app/mcp) is the live, documented agent
...
//	quotes, err := client.CallTool(ctx, "get_market_data", eagleeye.Args{"asset_class": "crypto"})
...
const UserAgent = "eagleeye-go/" + Version + " (+https://eagle-eye.app)"

const (
	DefaultBaseURL = "https://api.eagle-eye.app"
	DefaultMCPURL  = "https://eagle-eye.app/mcp"

	// APIKeyHeader is the header the API accepts for a user-issued key
	// (alias: X-Api-Key).
	APIKeyHeader = "X-EagleEye-Key"

	// MCPAuthErrorCode is the JSON-RPC error code the MCP server returns
	// when a call needs authentication.
	MCPAuthErrorCode = -32001

	authHint = "hint: this call needs a key - pass an API key to New or set EAGLEEYE_API_KEY (get one at https://eagle-eye.app/pro)"
...
func (e *APIError) Error() string {
	msg := fmt.Sprintf("eagleeye: HTTP %d: %s", e.Status, truncate(string(e.Body)))
...
func (e *MCPError) Error() string {
	msg := fmt.Sprintf("eagleeye: MCP error %d: %s", e.Code, e.Message)
...
// Client is a thin client for the Eagle Eye MCP server and REST API.
// The zero value is not usable; construct it with New.
type Client struct {
	// APIKey is the user API key sent as X-EagleEye-Key.
	APIKey string
	// BaseURL is the REST base (default https://api.eagle-eye.app).
	BaseURL string
	// MCPURL is the MCP endpoint (default https://eagle-eye.app/mcp).
	MCPURL string
...
// EAGLEEYE_API_KEY (or WM_API_KEY) environment variable; the REST and
// MCP endpoints honour EAGLEEYE_BASE_URL and EAGLEEYE_MCP_URL.
func New(apiKey string) *Client {
	if apiKey == "" {
		apiKey = os.Getenv("EAGLEEYE_API_KEY")
	}
	if apiKey == "" {
		apiKey = os.Getenv("WM_API_KEY")
	}
	baseURL := os.Getenv("EAGLEEYE_BASE_URL")
	if baseURL == "" {
		baseURL = DefaultBaseURL
	}
	mcpURL := os.Getenv("EAGLEEYE_MCP_URL")
	if mcpURL == "" {
...
func (c *Client) Get(ctx context.Context, path string, params Args) (json.RawMessage, error) {
	if !strings.HasPrefix(path, "/") {
		return nil, fmt.Errorf("eagleeye: Get needs a host-relative API path starting with %q", "/")
	}
	u := c.BaseURL + path
	if len(params) > 0 {
		q := url.Values{}
