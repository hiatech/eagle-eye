...
**PRO (hard-locked).** `premium: 'locked'` in `src/config/panels.ts:109`. `isPanelEntitled` unlocks it when **either** a valid `EAGLEEYE_API_KEY` is present **or** `isPro` is true (`market-implications` is in the `apiKeyPanels` allowlist at `src/config/panels.ts:973`).
