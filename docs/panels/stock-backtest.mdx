...
**PRO (hard-locked).** `premium: 'locked'` in the panel's constructor and in variant registrations. `isPanelEntitled` unlocks it when **either** a valid `EAGLEEYE_API_KEY` is present **or** `isPro` is true (`stock-backtest` is in the `apiKeyPanels` allowlist at `src/config/panels.ts:973`).
