...
**PRO (hard-locked).** `premium: 'locked'` on both variant registrations. `isPanelEntitled` unlocks it when **either** a valid `EAGLEEYE_API_KEY` is present **or** `isPro` is true (`stock-analysis` is in the `apiKeyPanels` allowlist at `src/config/panels.ts:973`).
