...
description: "Dynamic client registration, authorization, and token exchange endpoints that back the Eagle Eye MCP server's OAuth 2.1 authentication flow."
---

EagleEye runs a minimal OAuth 2.1 authorization server whose only client-facing purpose today is **granting access to the MCP server** at `/api/mcp`. It implements:
