...
description: "How user authentication, premium panel gating, and server-side session enforcement work in Eagle Eye — Clerk, cookies, and entitlement checks."
---

EagleEye uses [Clerk](https://clerk.com) for authentication. The auth system gates premium panels behind sign-in and tier checks, and enforces session-based access on server-side API endpoints via local JWT verification.
...
Desktop users with a valid `EAGLEEYE_API_KEY` in the Tauri keychain bypass all panel gating. The existing API key flow is unaffected — bearer tokens are a **second auth path**, not a replacement.
...
1. **Static API key** — `X-EagleEye-Key` header (existing flow, unchanged)
