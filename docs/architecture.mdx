...
description: "Design principles, intelligence tradecraft, and algorithmic decisions behind Eagle Eye — how the map-first dashboard is engineered and why."
---

> **Looking for the system reference?** See [`ARCHITECTURE.md`](https://github.com/hiatech/eagle-eye/blob/main/ARCHITECTURE.md) for deployment topology, directory layout, caching layers, CI/CD, and contributor how-to guides.
...
Eagle Eye currently uses **application localization**, not separate localized
...
Eagle Eye is written in vanilla TypeScript — no frontend framework (React, Vue, Svelte, Angular) is used. This is a deliberate architectural decision, not an oversight.
...
All six dashboard variants (Eagle Eye, Tech Monitor, Finance Monitor, Commodity Monitor, Happy Monitor, Energy Monitor) serve from a **single Vercel deployment**. The variant is determined at runtime by hostname detection:
...
| `eagle-eye.app` (default) | `full` |

On the desktop app, the variant is stored in `localStorage['eagleeye-variant']` and can be switched without rebuilding. The variant selector in the header bar navigates between deployed domains on the web or toggles the localStorage value on desktop.
...
Sentry captures unhandled exceptions and promise rejections in production, with environment-aware routing (production on `eagle-eye.app`, preview on `*.vercel.app`, disabled on localhost and Tauri desktop).
...
Transactions are sampled at 10% to balance observability with cost. Release tracking (`eagleeye@{version}`) enables regression detection across deployments.
...
The Service Status panel tracks the operational health of external services that EagleEye users may depend on.
