...
description: "Script country briefs, risk scores, and any of the 59 MCP tools from your shell with the official eagleeye npm CLI — no API integration to write."
---

[`eagleeye`](https://www.npmjs.com/package/eagleeye) is the official command-line client for the Eagle Eye global-intelligence API. It's a thin, **dependency-free** wrapper over the [MCP server](/mcp-overview) (the recommended agent surface) with a REST escape hatch, so you can pull briefs, risk scores, and conflict / cyber / market / news feeds from a shell script or an agent without writing a single line of HTTP plumbing. It ships as ESM and runs on **Node 18+**.

The source lives in [`cli/`](https://github.com/hiatech/eagle-eye/tree/main/cli) in the main repository and is published to npm on every `cli-v*` release tag.
...
Data commands map to MCP `tools/call` and require a user API key. Grab one at [eagle-eye.app/pro](https://www.eagle-eye.app/pro), then either pass it per-call or export it once:

```sh
export EAGLEEYE_API_KEY=wm_xxxxxxxx
eagleeye risk IR                       # reads the key from the environment
# — or —
eagleeye risk IR --api-key wm_xxxxxxxx # pass it explicitly
```

The key is sent as the `X-EagleEye-Key` header. See [Authentication](/usage-auth) for how keys, OAuth, and browser sessions differ, and [Rate Limits](/usage-rate-limits) for the per-plan allowances. If you call a data command with no key, the CLI exits `1` and prints a hint telling you exactly which flag to pass.
...
```sh
eagleeye country IR                              # AI strategic brief
eagleeye conflicts --country IR --limit 5        # recent conflict events
eagleeye markets --asset_class crypto            # crypto quotes
```
...
eagleeye call get_market_data --args '{"symbols":["AAPL","MSFT"]}'   # typed JSON args
eagleeye prompts        # list pre-built workflow prompt templates
eagleeye resources      # list addressable resource URIs
```
...
```sh
eagleeye health --api-key wm_xxx      # API status / health check (needs a key)
eagleeye get /api/cyber/v1/list-cyber-threats --api-key wm_xxx
eagleeye list cyber                   # list documented REST operations from the live spec
```
