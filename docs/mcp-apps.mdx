---
title: "MCP Apps: interactive ui:// widgets for EagleEye"
description: "Complete contract for EagleEye's MCP Apps interactive ui:// resources, tool links, host flow, security posture, and drift checks."
---

EagleEye supports [MCP Apps](https://modelcontextprotocol.io/extensions/apps/build) through the `io.modelcontextprotocol/ui` extension. The current fleet ships 10 MCP Apps: self-contained `ui://` HTML resources that an MCP Apps host can render inline after a linked tool call.
...
| Transport endpoint | `https://eagle-eye.app/mcp` |
| UI resource scheme | `ui://eagleeye/...` |
...
|---|---|---|---|
| `ui://eagleeye/country-risk.html` | `get_country_risk` | Country Risk (interactive) | Composite Instability Index score, component breakdown, travel advisory, and sanctions exposure. |
| `ui://eagleeye/world-brief.html` | `get_world_brief` | World Brief (interactive) | Precomputed citation-grounded global brief, grounding headlines, and source feed articles. |
| `ui://eagleeye/country-brief.html` | `get_country_brief` | Country Brief (interactive) | Per-country intelligence brief, analytical framework lens, and grounding sources. |
| `ui://eagleeye/market-radar.html` | `get_market_data` | Market Radar (interactive) | Fear & Greed composite plus equity, commodity, crypto, Gulf, and sector quote tables with signed, colour-coded change. |
| `ui://eagleeye/chokepoint-monitor.html` | `get_chokepoint_status` | Chokepoint Monitor (interactive) | Per-chokepoint transit summaries, week-over-week change, tanker split, and risk-level badge. |
| `ui://eagleeye/news-intelligence.html` | `get_news_intelligence` | News Intelligence (interactive) | AI-classified top stories with category, alert flag, country, and source. |
| `ui://eagleeye/conflict-events.html` | `get_conflict_events` | Conflict Events (interactive) | Active armed-conflict events (belligerents, violence type, country, fatalities, date) from UCDP. |
| `ui://eagleeye/natural-disasters.html` | `get_natural_disasters` | Natural Disasters (interactive) | Recent earthquakes (USGS magnitude, place, time) and active wildfires (NASA FIRMS), grouped. |
| `ui://eagleeye/prediction-markets.html` | `get_prediction_markets` | Prediction Markets (interactive) | Event-contract odds grouped by category (geopolitical, tech, finance) with a probability bar per market. |
| `ui://eagleeye/forecasts.html` | `get_forecast_predictions` | Forecasts (interactive) | AI-generated geopolitical and economic forecasts as probability cards (title, domain, region). |

## Runtime Flow

1. The host calls `initialize` on `https://eagle-eye.app/mcp`.
2. EagleEye returns the normal MCP capabilities plus `capabilities.extensions["io.modelcontextprotocol/ui"]`.
...
The view never fetches live EagleEye data itself. Live data always reaches the app through the host after a normal tool call.
...
|---|---|---|---|---|
| UI template | `resources/read` `ui://eagleeye/market-radar.html` | No | No | Static HTML shell, no data and no upstream fetch. |
| Public metadata | `resources/read` `eagleeye://seed-meta/freshness` | No | No | Metadata-only health/freshness probe. |
| Data template instantiation | `resources/read` `eagleeye://countries/de/risk` | Yes | Yes | Routes through the same dispatcher as the equivalent `tools/call`. |
