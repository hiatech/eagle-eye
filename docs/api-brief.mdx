...
description: "Read, share, render, and paginate the AI-composed Eagle Eye intelligence brief — endpoints for public shares, carousels, and share URLs."
---

EagleEye composes a per-user intelligence brief on Railway, stores each
...
Single-shot completion endpoint used by embedded widget iframes. Auth via `X-EagleEye-Key` (partner keys). Rate-limited per key.
