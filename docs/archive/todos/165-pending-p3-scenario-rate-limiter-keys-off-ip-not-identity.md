...
- See MEMORY: `feedback_is_caller_premium_trusted_origin.md` — the API key is extractable from `X-EagleEye-Key`
...
```ts
const apiKey = req.headers.get('x-eagleeye-key');
...
## Acceptance Criteria
- [ ] Rate limit key uses `X-EagleEye-Key` when present
