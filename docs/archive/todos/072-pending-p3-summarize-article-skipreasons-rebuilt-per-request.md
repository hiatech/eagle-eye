...
## Problem Statement
`server/eagleeye/news/v1/summarize-article.ts` lines 65-69 builds a `skipReasons` object on every invocation including hot-path cache hits:
```ts
...
## Technical Details
- File: `server/eagleeye/news/v1/summarize-article.ts:65-69, 89`
