...
## Findings
- **`server/eagleeye/intelligence/v1/deduct-situation.ts:29`** — cache key: `deduct:situation:v2:${sha256(query + '|' + geoContext)}` — framework absent
- **`server/eagleeye/intelligence/v1/get-country-intel-brief.ts:38-40`** — correct pattern: `const frameworkHash = frameworkRaw ? (await sha256Hex(frameworkRaw)).slice(0, 8) : ''; const cacheKey = \`ci-sebuf:v3:...${frameworkHash ? ':' + frameworkHash : ''}\``
...
## Technical Details
- File: `server/eagleeye/intelligence/v1/deduct-situation.ts`
- PR: koala73/worldmonitor#2380
- Related: `server/eagleeye/intelligence/v1/get-country-intel-brief.ts:38-40` (reference implementation)
