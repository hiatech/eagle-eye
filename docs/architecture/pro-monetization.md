...
Products are served at runtime from `https://api.eagle-eye.app/api/product-catalog`; generated client configuration lives in `pro-test/src/generated/tiers.json`:
...
- **Enterprise** — `mailto:enterprise@eagle-eye.app` (contact sales).
...
1. Validates `returnUrl` against an allow-listed set of eagle-eye.app origins.
...
- **Full-page return** (fallback / `/pro` path): Dodo redirects to `eagle-eye.app/?subscription_id=...&status=active`. `src/services/checkout-return.ts:handleCheckoutReturn()` reads params, cleans the URL, returns success boolean.
...
- **Share link**: `https://eagle-eye.app/pro?ref=<code>`.
