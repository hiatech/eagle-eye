...
description: "Deploying EAGLEEYE_API_KEY gating for the desktop app — cloud fallback, local-only fallback, and Convex-backed registration email capture."
---
## Overview

Desktop cloud fallback is gated on a `EAGLEEYE_API_KEY`. Without a valid key, the desktop app operates local-only (sidecar). A registration form collects emails via Convex DB for future key distribution.
...
|----------|-------------|---------|
| `EAGLEEYE_VALID_KEYS` | Comma-separated list of valid API keys | `wm_abc123def456,wm_xyz789` |
...
Add to `EAGLEEYE_VALID_KEYS` in Vercel dashboard (comma-separated, no spaces).
...
- `installRuntimeFetchPatch()` checks `EAGLEEYE_API_KEY` before allowing cloud fallback
...
- `api/_api-key.js` validates `X-EagleEye-Key` header on sebuf routes
- **Origin-aware**: desktop origins (`tauri.localhost`, `tauri://`, `asset://`) require a key
- Web origins (`eagle-eye.app`) pass through without a key
- Non-desktop origin with key header: key is still validated
- Invalid key returns `401 { error: "Invalid API key" }`

### CORS

`X-EagleEye-Key` is allowed in both `server/cors.ts` and `api/_cors.js`.
...
- [ ] Set `EAGLEEYE_VALID_KEYS` in Vercel
...
| `src/services/runtime-config.ts` | `EAGLEEYE_API_KEY` type, validation, `secretsReady` |
...
| `server/cors.ts` / `api/_cors.js` | CORS headers with `X-EagleEye-Key` |
| `src/components/EagleEyeTab.ts` | Settings UI for key + registration |
