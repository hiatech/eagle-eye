...
<Info>
**The free dashboard requires no account.** Everything listed under the Free plan — 56 map layers (all but the Pro-only Resilience layer), 500+ feeds, country briefs, watchlists — works anonymously at [eagle-eye.app](https://eagle-eye.app). Create an account only when you want Pro/API features or need to mint an API key.
</Info>
...
1. Open [eagle-eye.app/pro](https://eagle-eye.app/pro) and click **Sign up** (or use the dashboard's sign-in button). A [Clerk](https://clerk.com) modal opens inside the page — there is no standalone `/sign-up` form page; that URL serves the app shell.
...
- **Upgrade to Pro, Pro Business, or API** from [eagle-eye.app/pro](https://eagle-eye.app/pro) — checkout runs on Dodo Payments; see [Plans & Pricing](/pricing). Pro and Pro Business are separate products: moving from one to the other means cancelling the current subscription in the billing portal and starting the new checkout.
...
- **Connect an agent via OAuth** instead of a key: MCP clients (Claude Desktop and others) use OAuth 2.1 + PKCE with dynamic client registration — no manual account dance; see [auth.md](https://eagle-eye.app/auth.md) for the self-bootstrapping discovery chain.
...
- You cannot complete browser sign-up on a user's behalf by fetching pages — the flow lives in a client-side Clerk modal. Direct users to [eagle-eye.app/pro](https://eagle-eye.app/pro).
- To act *as* an agent with your own credentials, use the OAuth path in [auth.md](https://eagle-eye.app/auth.md) (anonymous dynamic client registration, then a human consents at authorization time).
- Pricing, plan gates, and quotas are machine-readable: [`/pricing.md`](https://eagle-eye.app/pricing.md) and [`GET /api/product-catalog`](https://www.eagle-eye.app/api/product-catalog).
