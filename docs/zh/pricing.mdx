...
Eagle Eye 提供一个免费的公开仪表盘，以及面向分析师工作流、API 访问和组织部署的付费层级。

<Info>
**机器可读来源**（面向 agent 的规范来源）：
- Markdown：[`/pricing.md`](https://eagle-eye.app/pricing.md)
- 实时 JSON（层级、价格、Dodo 产品 ID）：[`GET /api/product-catalog`](https://www.eagle-eye.app/api/product-catalog) — 公开，无需密钥
</Info>
...
| **Enterprise** | 定制 — [enterprise@eagle-eye.app](mailto:enterprise@eagle-eye.app) | 团队工作区、SSO/MFA/RBAC、白标、本地部署 / 气隙隔离 |
...
需要更高的限额？请在 [eagle-eye.app/pro](https://eagle-eye.app/pro) 升级，或联系 [enterprise@eagle-eye.app](mailto:enterprise@eagle-eye.app)。

## 购买和管理订阅

结账通过 [Dodo Payments](https://dodopayments.com) 完成；权益数据存储在 Convex 中。可从 [eagle-eye.app/pro](https://eagle-eye.app/pro) 订阅；在同一页面的客户门户中管理账单（发票、取消、续订）。端点详情：[商务端点](/zh/api-commerce)。
