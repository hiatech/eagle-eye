...
`GET /api/intelligence/v1/get-risk-scores`（proto `eagleeye.intelligence.v1.GetRiskScoresResponse`）暴露的 **综合不稳定指数（CII）** 和 **战略风险** 汇总的已发布方法论说明。

这些权重是**编辑性**的——由 EagleEye 情报团队基于对历史事件和当前地缘政治姿态的内部审阅撰写。它们**并非派生自已发布的学术指数**、同行评审论文或第三方风险产品。请将这些评分视为有态度的，而非经验性的。本说明的目的是使这些态度可被检查，以便下游消费者决定是否同意。
...
> **注意 — 评分契约变更。** 任何更改服务端/API 各国基线、事件乘数、战略风险汇总系数、内联服务端公式字面量、受保护的顶级公式常量或 CII 移动语义的编辑，必须伴随 `server/eagleeye/intelligence/v1/_risk-config.ts` 中 `CII_FORMULA_VERSION` 的提升，在同一提交中更新本文档，以及一条公开 CHANGELOG 条目。保留服务端/API 评分的真相源重构不提升传输版本。`tests/cii-scoring.test.mts` 中的测试强制本文档列出 `CURATED_COUNTRIES` 中的每个国家代码。
...
`server/eagleeye/intelligence/v1/get-risk-scores.ts` 中内嵌的国务院兜底表。因此每个 `CiiScore`
暴露：
...
这些是服务端 API 评分和前端客户端 CII 渲染都应用的编辑值。真相源是 [`shared/cii-weights.ts`](https://github.com/hiatech/eagle-eye/blob/main/shared/cii-weights.ts)；服务端和前端表从该模块派生，因此系数漂移在测试中失败而非静默发布。
...
[`server/eagleeye/intelligence/v1/_risk-config.ts`](https://github.com/hiatech/eagle-eye/blob/main/server/eagleeye/intelligence/v1/_risk-config.ts)。
它们的**理由**：
