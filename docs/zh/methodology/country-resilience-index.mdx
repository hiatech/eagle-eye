...
EagleEye 国家韧性指数（CRI）以 0-100 分对 196 国公开可排名范围进行评分，将长期结构能力与当前运营压力结合为可操作的韧性指标。上游静态/来源源可以包含更广泛的领土集合，但公开的 CRI 排名在分数发布前过滤到可排名范围。CRI 不依赖静态国家风险评级，而是每 6 小时从官方和权威来源更新，并公开完整的来源溯源、覆盖率和插补上下文，让分析师能准确看到分数*为什么*变动以及其中多少是真实数据与插补数据。
...
对于实时部署状态，请使用[运行时清单](https://api.eagle-eye.app/resilience/v1/get-runtime-manifest)。它报告活动公式标签、静态 `dataVersion`、排名缓存元数据、安全派生构造版本如 `constructVersions.energy`（`legacy` 或 `v2`），以及公共 `scoreInterval`/`rankStable` 支持路径的 `intervals` 可用性元数据，而不暴露部署标识符、原始 `RESILIENCE_*` 标志状态或内部缓存键。参考版仍是冻结的已发布包；不应将其视为当前生产中活动运行时状态的证据。
...
EagleEye 国家韧性指数以 0-100 分对 196 国公开可排名范围评分，覆盖 6 个领域和 20 个活跃维度（加上 2 个结构性退役维度为模式连续性在注册表中以 `coverage=0` 保留）。排名处理器仍可将低置信度或标题不合格的国家路由到 `greyedOut[]`，但可排名范围本身由提交的 UN 成员 + SAR 白名单固定。它将结构基线指标（治理质量、健康基础设施、财政能力）与实时压力信号（网络威胁、冲突事件、航运中断）和恢复能力指标（财政空间、储备、进口集中度）结合，产出每 6 小时更新的单一韧性分数。
...
权重总和为 1.00。权威值位于 `server/eagleeye/resilience/v1/_dimension-scorers.ts` 的 `RESILIENCE_DOMAIN_WEIGHTS`；如果此表与代码不一致，以代码为准。在活跃的 `pc` 公式中，绝对顶层影响力还取决于外层支柱权重。
...
关于完整构造理由和被拒绝的替代方案（项目权重分类、中转枢纽排除列表），请参见 [known-limitations.md § tradeSanctions → tradePolicy](https://github.com/hiatech/eagle-eye/blob/main/docs/methodology/known-limitations.md#tradesanctions--tradepolicy-ofac-domicile-component-dropped-ship-1-2026-04-25)。
...
关于完整构造理由、考虑的替代方案（项目权重分类、中转枢纽排除、单维公式重写、完全删除）和分阶段推出决策，请参见 [financial-system-exposure.md](https://github.com/hiatech/eagle-eye/blob/main/docs/methodology/financial-system-exposure.md)。
...
两个新维度的 `0.5` 权重将其对恢复分数的综合贡献限制在约 20%，符合计划中主权财富信号补充而非主导经典流动储备和财政空间信号的方向。权重通过 `server/eagleeye/resilience/v1/_dimension-scorers.ts` 中的 `RESILIENCE_DIMENSION_WEIGHTS` 应用；`_shared.ts` 中的 `coverageWeightedMean` 在计算领域均值前将每个维度的覆盖率乘以其权重，因此 `coverage=0`（退役）的维度无论权重如何仍贡献零。
...
当数据缺失时，系统用四类之一标记，使下游消费者可区分"没有发生"与"我们不知道"与"上游中断"与"维度不适用于此国家"。分类法定义在 `server/eagleeye/resilience/v1/_dimension-scorers.ts` 中作为导出的 `ImputationClass` 类型。
...
| 键 | 类型 | TTL | 写入方 | 读取方 |
|---|---|---|---|---|
| `resilience:score:v25:{countryCode}` | JSON | 6 小时 | `server/eagleeye/resilience/v1/_shared.ts` 中的 `buildResilienceScore` | `getResilienceScore` 处理器 |
...
| **来源质量** | 7.0 | 世界银行、IMF、WHO、IEA、UNHCR、UCDP、IPC、BIS、FAO、RSF、GPI：均权威。间隙：精选列表来源（BIS 约 40 经济体、WTO）不覆盖完整 EagleEye 国家集，这是 `unmonitored` 插补类存在的原因。Phase 2 T2.9 添加语言归一化信息信号以减少英语媒体偏差。 |
...
- **T2.3**（#2990/#3954）：三支柱聚合形状发布并激活。每个响应现在在 `pillars[]` 携带真实领域加权、覆盖率缩放支柱分数和支柱覆盖率。支柱权重：结构准备度 0.40、实时冲击暴露 0.35、恢复能力 0.25。惩罚因子 `(1 − α × (1 − min_pillar / 100))`，α = 0.5，作为 `penalizedPillarScore` 在 `server/eagleeye/resilience/v1/_shared.ts` 中实现，并由敏感度套件行使。**顶层 `overall_score` 是生产中的惩罚支柱组合形式**；六领域加权聚合保留为标志关闭回退路径。
...
**敏感度和比较工件**（2026-04-21，提交 `048bb8b`，52 国样本，在比较脚本修正为使用生产 `buildPillarList` 聚合后重新生成）：[`docs/snapshots/resilience-pillar-sensitivity-2026-04-21.json`](https://github.com/hiatech/eagle-eye/blob/main/docs/snapshots/resilience-pillar-sensitivity-2026-04-21.json)。
