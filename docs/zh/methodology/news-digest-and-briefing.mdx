---
title: "新闻摘要与简报方法论"
description: "EagleEye 如何采集 RSS 与 Atom 信息流、对新闻条目进行主题分类、按重要性评分、追踪话题报道趋势、过滤每日简报内容、调用 LLM 生成结构化摘要，并通过多源均衡与提示工程严格控制模型偏见与幻觉，构建一条透明、可审计的编辑管线与研究流程。"
---

## 范围

本页是新闻信息流摘要、每日摘要通知和 EagleEye Brief 的规范公开
...
信息流清单位于 `server/eagleeye/news/v1/_feeds.ts`。
接受的摘要变体为：
...
`energy` 是一个站点和客户端信息流变体（`energy.eagle-eye.app`），但它
...
每个已解析条目都从 `server/eagleeye/news/v1/_classifier.ts` 中的
关键词分类器开始。
...
- 信息流清单与摘要构建：`server/eagleeye/news/v1/_feeds.ts`，
  `server/eagleeye/news/v1/list-feed-digest.ts`
- 关键词分类器：`server/eagleeye/news/v1/_classifier.ts`
- API 契约：`proto/eagleeye/news/v1/list_feed_digest.proto`
