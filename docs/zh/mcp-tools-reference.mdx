---
title: "MCP 工具参考"
description: "EagleEye 通过 MCP 暴露的每个工具的逐工具详细参考文档 —— 完整涵盖输入参数与类型定义、数据新鲜度窗口、后端 API 端点映射，以及可直接复用的示例调用负载与响应片段，帮助开发者高效构建情报 Agent、工作流自动化与自定义客户端集成方案。"
---
...
如果您已完成 OAuth 流程，请在每个示例中将 `-H "X-EagleEye-Key: $WM_KEY"` 替换为 `-H "Authorization: Bearer $TOKEN"`。
...
来自 EagleEye 精选引导缓存的实时股票报价、大宗商品价格（包括黄金期货 GC=F）、加密货币价格、外汇汇率（USD/EUR、USD/JPY 等）、板块表现及估值覆盖、ETF 资金流和海湾市场报价。
...
- **类型：** 实时 RPC — 每次调用代理对 EagleEye API 的请求。Edge 运行时超时：**8.0s**。
...
- **类型：** 实时 RPC — 每次调用代理对 EagleEye API 的请求。最坏情况总预算 **~24s**（2s 上下文摘要抓取 + 22s 简报生成，顺序执行）。
...
来自 EagleEye 情报层的 AI 分类地缘政治威胁新闻摘要、GDELT 情报信号、跨源信号和安全公告。
...
通过枚举校验的 EagleEye 事件分类器，将提供的新闻标题或短文本分类为威胁类别和严重性。分类器为 temperature-0、按标题缓存 24 小时，且只会返回固定类别/级别枚举中的值 —— 绝不返回自由格式的 LLM 输出。无法产生枚举有效结果时 `classification` 为 `null`。
...
这是一项经过审慎权衡并已归档的立场，而非疏漏；完整决策以及撤回特定记录的运维路径参见 [`docs/architecture/intel-history-untrusted-text.md`](https://github.com/hiatech/eagle-eye/blob/main/docs/architecture/intel-history-untrusted-text.md)。
</Warning>
...
- **类型：** 实时 RPC — 每次调用代理对 EagleEye API 的请求。Edge 运行时超时：**25.0s**。
...
来自 EagleEye 预测模型的 AI 生成地缘政治和经济预测。涵盖即将到来的风险事件和概率评估。
