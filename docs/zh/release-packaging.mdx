...
description: "本指南提供 Eagle Eye 桌面二进制的可复现本地打包步骤，涵盖构建脚本、代码签名、公证、图标与安装包生成，帮助维护者与贡献者在 macOS、Windows 与 Linux 上产出与官方发布一致的桌面二进制，并对每一步进行确定性校验。"
---
本指南为桌面应用提供可复现的本地打包步骤。

**只发布一个二进制文件。** `Eagle Eye` 是唯一的桌面制品，所有变体 —— tech、finance、commodity、energy、happy —— 均在安装后于应用内切换，并在重启后保持。不存在按变体划分的安装包、标签或下载。

这是刻意的约束，而非疏漏：`/api/version` 和 `/api/download` 解析 `/releases/latest`，它最多只返回一个 release，因此第二条发布线永远无法送达用户（#5908）。

包标识由单一的 Tauri 配置控制：`src-tauri/tauri.conf.json` → `Eagle Eye` / `eagle-eye`。
...
`src-tauri/tauri.conf.json` → `Eagle Eye` / `eagle-eye`，其中 `bundle.icon` 指向共享的图标集。无论用户之后切换到哪个变体，每个打包构建都带有该标识，因此窗口标题与已安装的应用名称始终为 `Eagle Eye`。
