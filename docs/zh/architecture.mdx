---
title: "设计哲学"
description: "Eagle Eye 背后的设计原则、情报工艺与算法决策深度解读：全面介绍以地图为先的实时仪表盘是如何工程化实现的、为何选择这种技术架构，涵盖前端渲染管道、边缘缓存策略、Cloudflare Workers 数据采集流水线、评分引擎、实时推送与容灾降级机制等核心模块。"
---

> **在找系统参考？** 请参阅 [`ARCHITECTURE.md`](https://github.com/hiatech/eagle-eye/blob/main/ARCHITECTURE.md)，了解部署拓扑、目录结构、缓存层、CI/CD 以及贡献者操作指南。
...
Eagle Eye 使用原生 TypeScript 编写 — 不使用任何前端框架（React、Vue、Svelte、Angular）。这是一个刻意的架构决策，而非疏忽。
...
所有六个仪表盘变体（Eagle Eye、Tech Monitor、Finance Monitor、Commodity Monitor、Happy Monitor、Energy Monitor）从**单一 Vercel 部署**提供服务。变体在运行时通过主机名检测确定：
...
| `eagle-eye.app`（默认） | `full` |

在桌面应用上，变体存储在 `localStorage['eagleeye-variant']` 中，可在不重新构建的情况下切换。标题栏中的变体选择器在 Web 上在已部署域名间导航，或在桌面端切换 localStorage 值。
...
**每面板熔断器**（数据面板） — 从 API 端点抓取的面板使用 IndexedDB 支持的持久缓存（`eagleeye_persistent_cache` 存储）配 TTL 包络。抓取成功时，结果带过期时间戳存储。后续加载时，熔断器立即提供缓存结果并尝试后台刷新。如果后台刷新失败，过期的缓存数据继续显示 — 面板不会因瞬时 API 故障而空白。缓存条目在页面重载和浏览器重启后保留。
...
Sentry 在生产环境中捕获未处理异常和 Promise 拒绝，具备环境感知路由（生产环境在 `eagle-eye.app`，预览环境在 `*.vercel.app`，localhost 和 Tauri 桌面端禁用）。
...
事务以 10% 采样以在可观测性与成本间平衡。发布追踪（`eagleeye@{version}`）支持跨部署的回归检测。
...
服务状态面板追踪 EagleEye 用户可能依赖的外部服务的运行健康度。
