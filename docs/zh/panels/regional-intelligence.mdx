...
**PRO（硬锁定）且需手动启用。** `src/config/panels.ts:110` 中的 `enabled: false, premium: 'locked'`。当存在有效的 `EAGLEEYE_API_KEY` **或** `isPro` 为 true 时，`isPanelEntitled` 解锁该面板（`regional-intelligence` 在 `src/config/panels.ts:973` 的 `apiKeyPanels` 允许列表中）。
