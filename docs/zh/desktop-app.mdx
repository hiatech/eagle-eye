---
title: "桌面应用"
description: "Eagle Eye Tauri 桌面应用架构深度详解：涵盖 Rust sidecar 进程管理、系统密钥链安全存储、云端 API 回退策略、离线缓存机制、自动更新流程，以及 Windows、macOS、Linux 的多平台构建、代码签名与公证发布流水线的完整实现细节。"
---
...
- **云端回退** —— 当本地 API 处理器失败或缺失时，请求会透明地回退到云端部署（eagle-eye.app），并剥离 Origin 头
...
Eagle Eye 桌面端使用一个运行时配置 schema，具备按功能切换的开关与密钥支撑的凭据。
...
- `OLLAMA_API_URL`
- `OLLAMA_MODEL`
- `EAGLEEYE_API_KEY` —— 控制云端回退访问（最少 16 字符）
- `WTO_API_KEY`
- `AVIATIONSTACK_API`
- `ICAO_API_KEY`
- `UCDP_ACCESS_TOKEN`
...
桌面构建通过 Tauri 命令绑定将密钥持久化到 OS 凭据存储中，底层为 Rust `keyring` 条目（`eagle-eye` 服务命名空间）。
...
- Eagle Eye API 密钥：云端回退被阻塞；桌面端仅以本地模式运行。
...
桌面应用通过轮询 `eagle-eye.app/api/version` 检查新版本 —— 启动时一次（延迟 5 秒），之后每 6 小时一次。当检测到较新版本时（semver 比较），会出现一个非侵入式的更新徽章，附带指向 GitHub Release 页面的直接链接。

更新提示可按版本忽略 —— 忽略 v2.5.0 不会抑制 v2.6.0 的通知。只发布一个 Eagle Eye 二进制文件，所有变体均在应用内切换，因此更新器仅按操作系统和架构解析下载（`/api/download?platform=…`），而不按变体解析。
