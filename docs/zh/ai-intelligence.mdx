---
title: "AI 情报"
description: "深入解析驱动 Eagle Eye AI 分析栈的核心技术组件：包含多层 LLM 提示链、RAG 检索增强流水线、威胁分类与实体识别模型、事件推演引擎，以及运行于浏览器端的轻量 ML 推理，用于实时生成国家情报简报、事件摘要、关联分析、异常检测与动态风险评分。"
---
...
标题记忆并不是唯一的记忆系统。Eagle Eye 还会保存一份需要 Pro 权限的服务端历史，收录冲突、军事与能源事件，使 agent 和 API 客户端能够跨会话、跨设备检索证据。
...
| `extract_entities` | Eagle Eye 的公司、市场、国家、领导人、CVE/APT 与 FIN 注册表 | 确定性；接受有长度上限的调用方文本或当前摘要 |
