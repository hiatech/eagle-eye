...
description: "支撑 Eagle Eye MCP 服务器 OAuth 2.1 认证流程的完整 API 参考：涵盖动态客户端注册、授权码颁发、访问令牌交换与刷新端点，为 AI 智能体、LLM 客户端、Claude Desktop 与第三方集成提供符合规范的安全访问、权限范围控制与令牌生命周期管理。"
---

EagleEye 运行一个最小化的 OAuth 2.1 授权服务器,目前其面向客户端的唯一用途是**授予对 `/api/mcp` MCP 服务器的访问权限**。它实现了:
