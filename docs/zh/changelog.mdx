---
title: "更新日志"
description: "Eagle Eye 所有版本重要变更的完整发布记录：按语义化版本号有序组织，涵盖新增功能、错误修复、性能优化、UI 改进、API 新增与弃用端点、破坏性改动说明、安全公告及迁移指南，方便开发者、集成方与订阅用户跟踪产品演进、规划升级路径与查阅历史发布备注。"
rss: true
---

Eagle Eye 的所有重要变更均记录于此。通过 [RSS](https://www.eagle-eye.app/docs/changelog/rss.xml) 订阅以获取最新动态。
...
- **CDN 缓存绕过已修复**：`CDN-Cache-Control` 头现仅对受信任来源（eagle-eye.app、Vercel 预览、Tauri）发送。无来源的服务器端请求始终调用边缘函数以执行 `validateApiKey`，防止外部爬虫获得缓存的 200 响应。
...
- Story 页头使用完整域名 EAGLE-EYE.APP（#799）
...
- **边缘缓存**：proxy.eagle-eye.app（#478）与 api.eagle-eye.app（#471）的 Cloudflare 边缘缓存
...
- **金融变体**：新增专用市场优先变体（`finance.eagle-eye.app`），附带以金融/交易为核心的数据源、面板与地图默认值
...
- **CORS 加固**：收紧 Vercel 预览部署正则以阻断来源伪造（`eagleeyeEVIL.vercel.app`）
