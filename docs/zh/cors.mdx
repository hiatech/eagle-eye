---
title: "CORS"
description: "Eagle Eye 跨域请求保护机制的完整技术说明，以及在添加新 API 端点时需要注意的处理方式：涵盖域名允许列表配置、预检 OPTIONS 请求处理、Cloudflare Worker 门控层、凭据传递策略、公开与私有端点区分以及常见 CORS 错误排查。"
---
...
| 模式 | 匹配 |
| --- | --- |
| `(*.)?eagle-eye.app` | 生产环境 + 子域名（`tech.`、`finance.` 等） |
| `eagleeye-*-elie-*.vercel.app` | Vercel 预览部署 |
...
- `Content-Type`
- `Authorization`
- `X-EagleEye-Key`（用于桌面/第三方访问的 API key）。密钥管理详情请参见 [API Key 门控](/zh/api-key-deployment)。
- `X-Api-Key`
- `X-Widget-Key`
- `X-Pro-Key`
