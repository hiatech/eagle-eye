---
title: "MCP Apps：EagleEye 的交互式 ui:// 小部件"
description: "EagleEye MCP Apps 完整实现契约文档，覆盖交互式 ui:// 资源、工具链接协议、宿主客户端渲染流程、沙箱与安全态势、能力协商、以及漂移检查机制，帮助集成开发者理解在 Claude、Cursor 等 MCP 客户端中嵌入可交互 UI 组件的规范与调试流程。"
---

EagleEye 通过 `io.modelcontextprotocol/ui` 扩展支持 [MCP Apps](https://modelcontextprotocol.io/extensions/apps/build)。当前阵容提供 10 个 MCP Apps：自包含的 `ui://` HTML 资源，MCP Apps 宿主可在关联的工具调用后内联渲染它们。
...
|---|---|---|---|
| `ui://eagleeye/country-risk.html` | `get_country_risk` | 国家风险（交互式） | 综合不稳定指数评分、组成部分细分、旅行建议和制裁风险敞口。 |
| `ui://eagleeye/world-brief.html` | `get_world_brief` | 世界简报（交互式） | AI 摘要的全球简报、支撑性头条新闻和信息源文章。 |
| `ui://eagleeye/country-brief.html` | `get_country_brief` | 国家简报（交互式） | 按国家划分的情报简报、分析框架视角和支撑来源。 |
| `ui://eagleeye/market-radar.html` | `get_market_data` | 市场雷达（交互式） | 恐惧贪婪综合指数，以及股票、大宗商品、加密货币、海湾地区和行业报价表，附带带符号、颜色编码的涨跌。 |
| `ui://eagleeye/chokepoint-monitor.html` | `get_chokepoint_status` | 咽喉要道监视器（交互式） | 按咽喉要道划分的通行摘要、周环比变化、油轮构成和风险等级徽章。 |
| `ui://eagleeye/news-intelligence.html` | `get_news_intelligence` | 新闻情报（交互式） | AI 分类的头条报道，附带类别、警报标记、国家和来源。 |
| `ui://eagleeye/conflict-events.html` | `get_conflict_events` | 冲突事件（交互式） | 来自 UCDP 的活跃武装冲突事件（交战方、暴力类型、国家、死亡人数、日期）。 |
| `ui://eagleeye/natural-disasters.html` | `get_natural_disasters` | 自然灾害（交互式） | 近期地震（USGS 震级、地点、时间）和活跃野火（NASA FIRMS），分组展示。 |
| `ui://eagleeye/prediction-markets.html` | `get_prediction_markets` | 预测市场（交互式） | 按类别（地缘政治、科技、金融）分组的事件合约赔率，每个市场附带一个概率条。 |
| `ui://eagleeye/forecasts.html` | `get_forecast_predictions` | 预测（交互式） | 以概率卡片形式呈现的 AI 生成的地缘政治和经济预测（标题、领域、地区）。 |

## 运行时流程

1. 宿主对 `https://eagle-eye.app/mcp` 调用 `initialize`。
2. EagleEye 返回正常的 MCP 能力，外加 `capabilities.extensions["io.modelcontextprotocol/ui"]`。
...
视图本身从不获取实时的 EagleEye 数据。实时数据始终在正常的工具调用之后通过宿主到达应用。
...
|---|---|---|---|---|
| UI 模板 | `resources/read` `ui://eagleeye/market-radar.html` | 否 | 否 | 静态 HTML 外壳，无数据、无上游获取。 |
| 公开元数据 | `resources/read` `eagleeye://seed-meta/freshness` | 否 | 否 | 仅含元数据的健康/新鲜度探测。 |
| 数据模板实例化 | `resources/read` `eagleeye://countries/de/risk` | 是 | 是 | 经由与等效 `tools/call` 相同的调度器路由。 |
