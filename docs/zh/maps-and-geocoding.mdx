---
title: "地图基础设施与地理编码"
description: "Eagle Eye 地图图层所用的静态地图资源、国家与地区几何数据、正向与反向地理编码服务、Mapbox 与自建瓦片配置，以及边界叠加层、投影与坐标系配置的完整参考文档，帮助开发者深入了解底层数据源、缓存策略与自定义扩展地图渲染管线的关键接入点和调优方法。"
---

## R2 CDN — `maps.eagle-eye.app`

所有大型静态地图文件均通过 Cloudflare R2 提供，而**不是** Vercel。R2 存储桶 `eagleeye-maps` 由 CF 代理的自定义域名提供服务：

| URL | 用途 |
|-----|-----|
| `https://maps.eagle-eye.app/<file>` | 生产环境 URL（CF 代理、缓存、CORS 头） |
...
2. `country-boundary-overrides.geojson` — 来自 [Natural Earth](https://www.naturalearthdata.com/) 的可选更高分辨率多边形，从 R2 CDN（`maps.eagle-eye.app`）提供。通过 `ISO3166-1-Alpha-2`（或 `ISO_A2`）代码匹配；匹配到的要素替换基础几何数据
...
PMTiles 也通过 `maps.eagle-eye.app` 从 R2 提供，通过 `VITE_PMTILES_URL` 配置。

## 常见错误

| 错误 | 修复 |
|---------|-----|
| 在代码中使用 `pub-*.r2.dev` URL | 始终使用 `maps.eagle-eye.app`（CF 代理） |
