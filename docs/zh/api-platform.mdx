---
title: "平台端点"
description: "Eagle Eye 平台基础设施端点完整参考：涵盖引导启动、健康检查、版本探测、缓存清理与用户偏好设置等 RPC——这些是每个仪表盘客户端、桌面应用、命令行工具、SDK 与 MCP 服务器在启动握手、运行时同步、遥测上报与退出清理阶段都会调用的底层管道 API。"
---
...
- **认证**:浏览器 `wm-session` cookie、`X-EagleEye-Key` 或 `X-Api-Key` 别名。用户签发的密钥会针对当前 API 访问权限进行验证。
- **匿名天气**:`?keys=weatherAlerts` 仅在**未发送任何 API 密钥头时**公开。如果你附加 `X-EagleEye-Key` / `X-Api-Key`,即使是天气请求也会被完全验证 —— 格式错误的密钥返回 `401`,没有当前 API 访问权限的密钥返回 `403`。始终发送密钥的调用方必须发送有效的、有权限的密钥(或省略该头以使用匿名天气路径)。该 URL 为 `no-store`,这正是该约定在边缘层与源站同样成立的原因:没有任何缓存持有它,因此无效密钥永远不会被已预热的匿名缓存条目应答。
...
- **服务器到服务器**:直接用 `X-EagleEye-Key: wm_...` 调用 `https://api.eagle-eye.app/api/bootstrap`。此端点没有单独的网关主机、令牌交换、激活步骤或 IP 白名单要求。
...
桌面端只发布一个二进制文件,所有变体均在安装后于应用内切换,因此每个受支持的值都会解析到所请求平台的同一个 Eagle Eye 资产 —— 该参数只记录调用方来自哪个变体,并不会选择不同的下载。无法识别的值将重定向到 [releases/latest](https://github.com/koala73/worldmonitor/releases/latest),且不会调用 GitHub。省略 `variant` 时的解析方式与传入受支持值时完全相同 —— 身份过滤在所有路径上都会生效,因此发行版中仅仅匹配平台后缀的无关资产永远不会被选中。
...
- `X-EagleEye-Desktop-Timestamp`:Unix epoch 毫秒,与服务器时间相差不超过 5 分钟。
- `X-EagleEye-Desktop-Signature`:`sha256=<hex HMAC-SHA256>`。
