---
title: "API 版本控制与弃用"
description: "EagleEye REST API 的兼容性保证、弃用通知和停用信号，供智能体和长期集成使用。"
---

EagleEye 在 URL 路径中标明公共 REST API 的版本：
...
在已发布的主版本内，EagleEye 可能会新增可选的请求字段、响应字段、操作和枚举值。
现有字段的含义和类型保持不变。除非发布新的主版本路径，否则我们不会删除或重命名
操作或字段、将可选字段改为必填字段，或有意引入其他破坏性变更。

客户端应忽略无法识别的响应字段和枚举值。已打包的
[OpenAPI 规范](https://www.eagle-eye.app/openapi.yaml)是当前已发布契约的权威来源。

## 弃用时间表

当 EagleEye 替换或停用公共 REST 版本或操作时：
