...
The **scenarios** API 是建立在 EagleEye 咽喉点 + 贸易数据集之上的 PRO 专属、作业排队接口。调用方将命名的场景模板加入队列（可选国家），然后轮询 job-id 直到 worker 完成。若未提供国家，v1 仅在种子报告集上计算：`US`、`CN`、`RU`、`IR`、`IN` 和 `TW`。

<Info>
本服务由 proto 支持并已纳入已发布的 OpenAPI 包 — 参见 `proto/eagleeye/scenario/v1/service.proto` 和 `/api/ScenarioService.openapi.yaml`。本页在生成的参考文档之上补充迁移说明和示例。
</Info>
...
**响应** — 使用已上线的模板之一（`server/eagleeye/supply-chain/v1/scenario-templates.ts`）的简略示例：
```json
{
  "templates": [
    {
