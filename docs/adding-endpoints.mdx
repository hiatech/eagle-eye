...
description: "All JSON API endpoints in Eagle Eye must use sebuf. This guide walks through adding a new RPC to an existing service and adding an entirely new service."
---
All JSON API endpoints in Eagle Eye **must** use sebuf. Do not create standalone `api/*.js` or `api/*.ts` files for new data APIs — the legacy pattern is deprecated and being removed.

This guide walks through adding a new RPC to an existing service and adding an entirely new service.

> **Enforcement:** `npm run lint:api-contract` runs in CI (see `.github/workflows/lint-code.yml`). It walks every file under `api/`, pairs each sebuf gateway (`api/<domain>/v<N>/[rpc].ts`) with a generated service under `src/generated/server/eagleeye/`, and rejects any file that is neither a gateway nor listed in `api/api-route-exceptions.json`. The manifest is the only escape hatch for endpoints that genuinely cannot be proto — OAuth callbacks, binary responses, upstream proxies, operator plumbing — and every entry is pinned to @SebastienMelki via `.github/CODEOWNERS`. Expect reviewer pushback on new entries.
>
...
Create `proto/eagleeye/seismology/v1/get_earthquake_details.proto`:

```protobuf
syntax = "proto3";
package eagleeye.seismology.v1;

import "buf/validate/validate.proto";
import "eagleeye/seismology/v1/earthquake.proto";
...
Edit `proto/eagleeye/seismology/v1/service.proto`:

```protobuf
import "eagleeye/seismology/v1/get_earthquake_details.proto";
...
Create `server/eagleeye/seismology/v1/get-earthquake-details.ts`:
...
Edit `server/eagleeye/seismology/v1/handler.ts`:
...
```
proto/eagleeye/weather/v1/
```

### 2. Define entity messages

Create `proto/eagleeye/weather/v1/weather_station.proto`:

```protobuf
syntax = "proto3";
package eagleeye.weather.v1;
...
Create `proto/eagleeye/weather/v1/list_weather_stations.proto`:
...
import "buf/validate/validate.proto";
import "eagleeye/core/v1/pagination.proto";
import "eagleeye/weather/v1/weather_station.proto";
...
message ListWeatherStationsRequest {
  ...
  eagleeye.core.v1.PaginationRequest pagination = 3;
}
...
message ListWeatherStationsResponse {
  ...
  // Pagination metadata.
  eagleeye.core.v1.PaginationResponse pagination = 2;
}
```

### 4. Define the service

Create `proto/eagleeye/weather/v1/service.proto`:
...
import "sebuf/http/annotations.proto";
import "eagleeye/weather/v1/list_weather_stations.proto";
...
```
server/eagleeye/weather/v1/
├── handler.ts                     # thin re-export
└── list-weather-stations.ts       # RPC implementation
```

`server/eagleeye/weather/v1/list-weather-stations.ts`:
```typescript
import type {
...
} from '../../../../src/generated/server/eagleeye/weather/v1/service_server';
...
`server/eagleeye/weather/v1/handler.ts`:
```typescript
import type { WeatherServiceHandler } from '../../../../src/generated/server/eagleeye/weather/v1/service_server';
...
import { createWeatherServiceRoutes } from '../../../src/generated/server/eagleeye/weather/v1/service_server';
import { weatherHandler } from '../../../server/eagleeye/weather/v1/handler';
...
} from '@/generated/client/eagleeye/weather/v1/service_client';
...
| Type | Import | Use for |
|------|--------|---------|
| `GeoCoordinates` | `eagleeye/core/v1/geo.proto` | Any lat/lon location (has built-in -90/90 and -180/180 bounds) |
| `BoundingBox` | `eagleeye/core/v1/geo.proto` | Spatial filtering |
| `TimeRange` | `eagleeye/core/v1/time.proto` | Time-based filtering (has `INT64_ENCODING_NUMBER`) |
| `PaginationRequest` | `eagleeye/core/v1/pagination.proto` | Request pagination (has page_size 1-100 constraint) |
| `PaginationResponse` | `eagleeye/core/v1/pagination.proto` | Response pagination metadata |
