---
title: "Rate Limits"
description: "Per-endpoint, per-key, and per-IP rate limits across the Eagle Eye API — headers, 429 handling, and backoff guidance for production clients."
---
...
EagleEye does **not** automatically upgrade a user, charge for overages, or move a customer into API Business because they crossed a cap. Any future hard enforcement for paid plans must first pass the internal `apiPlanLimitNotices.getEnforcementReadiness` gate: no stale usage source, no pending/failed email, and no blocked self-serve upgrade path.
