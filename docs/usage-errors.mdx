---
title: "Errors"
description: "Standard Eagle Eye API error shape, common HTTP status codes, JSON-RPC error envelopes, and safe retry strategies for transient failures."
---
...
- Every edge response includes `x-vercel-id` and `x-eagleeye-deploy` headers — include those when reporting issues.
- Sentry alerts forward to [status.eagle-eye.app](https://status.eagle-eye.app/).
