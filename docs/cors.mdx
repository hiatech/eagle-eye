---
title: "CORS"
description: "How cross-origin request protection works in Eagle Eye and what to do when adding new API endpoints — allowlists, preflights, and Worker gate."
---
...
| Pattern | Matches |
| --- | --- |
| `(*.)?eagle-eye.app` | Production + subdomains (`tech.`, `finance.`, etc.) |
| `eagleeye-*-elie-*.vercel.app` | Vercel preview deploys |
...
- `Content-Type`
- `Authorization`
- `X-EagleEye-Key` (API key for desktop/third-party access). See [API Key Gating](/api-key-deployment) for key management details.
- `X-Api-Key`
- `X-Widget-Key`
- `X-Pro-Key`
