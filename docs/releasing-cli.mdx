...
description: "How maintainers publish the eagleeye npm CLI from cli/ using cli-v* tags, npm trusted publishing, and the release workflow's dry-run gates."
---
This runbook covers maintainer releases of the official [`eagleeye`](https://www.npmjs.com/package/eagleeye) npm CLI from `cli/`. CLI releases are intentionally independent from desktop app releases: a `cli-vX.Y.Z` Git tag triggers `.github/workflows/publish-cli.yml`.
...
- The `eagleeye` package already exists on npm.
...
After the workflow succeeds, confirm the new version appears on npm and that `npx eagleeye --version` resolves to the published version.
