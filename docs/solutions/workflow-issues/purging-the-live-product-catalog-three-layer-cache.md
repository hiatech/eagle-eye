...
severity: medium
applies_when: "Catalog copy or pricing changed in a merged PR but https://api.eagle-eye.app/api/product-catalog still serves the old payload"
symptoms:
...
- A pricing/catalog PR merged but `https://api.eagle-eye.app/api/product-catalog` serves old tiers.
...
```bash
curl -s "https://api.eagle-eye.app/api/product-catalog?cb=$RANDOM" -A "Mozilla/5.0" \
