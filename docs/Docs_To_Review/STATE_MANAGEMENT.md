...
Eagle Eye is an AI-powered real-time global intelligence dashboard built with **vanilla TypeScript** — no framework, no reactive stores. All state is managed manually through class properties, `localStorage`, `IndexedDB`, and URL query parameters.
...
Stored in `localStorage` under `eagleeye-panels` as `Record<string, PanelConfig>`. Includes per-panel `enabled` state, `name`, and `priority`. Controlled by the variant config and user overrides.
...
| Key | Type | Purpose |
|-----|------|---------|
| `eagleeye-panels` | `Record<string, PanelConfig>` | Per-panel enabled/name/priority |
| `eagleeye-monitors` | `Monitor[]` | Color/label configs for monitors |
| `eagleeye-layers` | `MapLayers` | Enabled/disabled map layer toggles |
| `eagleeye-disabled-feeds` | `string[]` | User-disabled news feed sources |
...
sequenceDiagram
    ...
    setTheme->>localStorage: setItem('eagleeye-theme', 'light')
...
export async function initDB(): Promise<IDBDatabase> {
  // Opens or creates eagleeye_db v1
...
- **Toggles key:** `eagleeye-runtime-feature-toggles` in `localStorage`
...
Complete reference of every `localStorage` key used by Eagle Eye:

| Key | Purpose | Format | Source |
|-----|---------|--------|--------|
| `eagleeye-variant` | Active variant override | `'full' \| 'tech' \| 'finance'` | `src/config/variant.ts`, `App.ts` |
| `eagleeye-theme` | Theme preference | `'dark' \| 'light'` | `src/utils/theme-manager.ts` |
| `panel-order` | Panel arrangement order | `string[]` (JSON) | `App.ts` |
| `eagleeye-panel-spans` | Panel grid sizes | `Record<string, number>` (JSON) | `src/components/Panel.ts` |
| `eagleeye-panels` | Panel enabled/config state | `Record<string, PanelConfig>` (JSON) | `STORAGE_KEYS.panels` |
| `eagleeye-monitors` | Monitor color/label configs | `Monitor[]` (JSON) | `STORAGE_KEYS.monitors` |
| `eagleeye-layers` | Map layer toggles | `MapLayers` (JSON) | `STORAGE_KEYS.mapLayers` |
| `eagleeye-disabled-feeds` | User-disabled news sources | `string[]` (JSON) | `STORAGE_KEYS.disabledFeeds` |
| `eagleeye-runtime-feature-toggles` | Desktop feature toggles | `Record<RuntimeFeatureId, boolean>` (JSON) | `src/services/runtime-config.ts` |
| `eagleeye-persistent-cache:{key}` | Persistent data cache entries | `CacheEnvelope<T>` (JSON) | `src/services/persistent-cache.ts` |
| `wm-update-dismissed-{version}` | Dismissed update notifications | `'1'` | `App.ts` |
| `eagleeye-panel-order-v1.9` | Panel order migration flag | `'done'` | `App.ts` (one-time migration) |
| `eagleeye-tech-insights-top-v1` | Tech variant migration flag | `'done'` | `App.ts` (one-time migration) |
