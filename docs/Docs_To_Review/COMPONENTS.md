# Component Documentation — Eagle Eye
...
Eagle Eye's UI is built entirely with **vanilla TypeScript** — no React, Vue, or
...
`localStorage['eagleeye-panel-spans']` as a JSON map of `{ [id]: height }`.
...
Eagle Eye ships two independent map renderers and an adapter that picks the
...
Recent selections stored in `localStorage['eagleeye_recent_searches']`
(most recent 10).
