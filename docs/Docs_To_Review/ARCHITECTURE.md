...
| **IndexedDB** | `eagleeye_db` | Client-side storage for playback snapshots and temporal baseline data. |
...
Eagle Eye ships as three product variants from a single codebase. Each variant surfaces a different subset of panels, map layers, and data sources.

| Variant | Domain | Focus |
|---|---|---|
| `full` | eagle-eye.app | Geopolitics, military, OSINT, conflicts, markets |
| `tech` | tech.eagle-eye.app | AI/ML, startups, cybersecurity, developer tools |
| `finance` | finance.eagle-eye.app | Markets, trading, central banks, macro indicators |
...
```
localStorage('eagleeye-variant')  →  import.meta.env.VITE_VARIANT  →  default 'full'
```
...
export const SITE_VARIANT: string = (() => {
  if (typeof window !== 'undefined') {
    const stored = localStorage.getItem('eagleeye-variant');
...
graph TD
    ...
    subgraph LayerConfig["Layer Configuration"]
        ...
        UserPref["localStorage overrides<br/>(eagleeye-layers)"]
...
2. **User localStorage** — Stored under the key `eagleeye-layers`. Users toggle layers in the map controls UI, and their preferences persist across sessions.
...
Eagle Eye employs a five-tier caching strategy to minimise API costs, reduce latency, and enable offline operation.
...
graph TD
    ...
    subgraph Tier5["Tier 5: Persistent Cache"]
        ...
        LSFallback["localStorage fallback<br/>prefix: eagleeye-persistent-cache:"]
        PC --> TauriInvoke
        PC --> LSFallback
    end
...
The `eagleeye_db` IndexedDB database contains two object stores:
...
**2. Feature Toggles** — 14 `RuntimeFeatureId` values stored in localStorage under the key `eagleeye-runtime-feature-toggles`:
...
Eagle Eye runs machine-learning inference directly in the browser using ONNX Runtime Web via Transformers.js, with API-based fallbacks for constrained devices.
...
Eagle Eye uses a circuit-breaker pattern to manage transient failures across its many data sources, preventing cascade failures and providing graceful degradation.
