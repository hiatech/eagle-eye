...
> **Eagle Eye** — Config-driven panel architecture powering three site variants.
>
...
4. [Full Variant Panels](#4-full-variant-panels-eagleeyeio)
5. [Tech Variant Panels](#5-tech-variant-panels-techeagleeyeio)
6. [Finance Variant Panels](#6-finance-variant-panels-financeeagleeyeio)
...
Eagle Eye uses a **config-driven panel system** where every dashboard tile — from live news feeds to AI insights to market data — is declared as a `PanelConfig` entry inside a variant-specific configuration object. The system is designed around three principles:
...
|---------|--------|-------|-------------|
| `full` | eagle-eye.app | Geopolitical intelligence, OSINT, defense | 44 |
| `tech` | tech.eagle-eye.app | Technology, AI/ML, startups, VC | Tech-focused |
| `finance` | finance.eagle-eye.app | Markets, trading, macro, commodities | Finance-focused |
| `commodity` | commodity.eagle-eye.app | Mining, metals, energy, supply chains | Commodity-focused |
| `happy` | happy.eagle-eye.app | Positive news, breakthroughs, conservation | Happy-focused |
| `energy` | energy.eagle-eye.app | Oil, gas, chokepoints, energy security | Energy-focused |
...
export const STORAGE_KEYS = {
  panels:        'eagleeye-panels',          // Panel visibility toggles
  monitors:      'eagleeye-monitors',         // Monitor keyword configs
  mapLayers:     'eagleeye-layers',           // Map layer toggles
  disabledFeeds: 'eagleeye-disabled-feeds',   // Per-source feed disabling
} as const;
```
...
|-----|---------|------------|
| `eagleeye-panel-spans` | Panel height/span sizes (1–4) | `Panel.ts` |
| `panel-order` | Drag-and-drop panel ordering | `App.ts` |
| `eagleeye-variant` | Last-active variant (triggers reset on change) | `App.ts` |
| `eagleeye-panel-order-v1.9` | Migration flag for v1.9 panel layout | `App.ts` |
...
Span values are persisted per-panel in the `eagleeye-panel-spans` localStorage key as a JSON object `{ [panelId]: spanNumber }`.
...
## 4. Full Variant Panels (eagle-eye.io)
...
## 5. Tech Variant Panels (tech.eagle-eye.io)
...
## 6. Finance Variant Panels (finance.eagle-eye.io)
...
|---------|-----------------|--------|----------------|:--------------:|
| Panel visibility | `eagleeye-panels` | `Record<string, PanelConfig>` JSON | `DEFAULT_PANELS` | ✅ |
| Panel ordering | `panel-order` | `string[]` JSON | Config declaration order | ✅ |
| Panel sizes/spans | `eagleeye-panel-spans` | `Record<string, number>` JSON | All span-1 | ✅ |
| Map layer toggles | `eagleeye-layers` | `MapLayers` JSON | `DEFAULT_MAP_LAYERS` | ✅ |
| Monitor keywords | `eagleeye-monitors` | `Monitor[]` JSON | `[]` | ✅ |
| Disabled sources | `eagleeye-disabled-feeds` | `string[]` JSON | `[]` | ✅ |
| Active variant | `eagleeye-variant` | Plain string | `SITE_VARIANT` | ✅ |
| Banner dismissal | `banner-dismissed` (sessionStorage) | Timestamp string | — | Session only |

### 9.2 Variant Change Reset

When the stored variant (`eagleeye-variant`) differs from the current `SITE_VARIANT`, the App constructor performs a full reset:
...
This ensures users switching between variant domains (e.g. from eagle-eye.io to tech.eagle-eye.io) get a clean default experience for the new variant.
...
Step-by-step guide to adding a new panel to Eagle Eye.
...
sequenceDiagram
    ...
    App->>localStorage: saveToStorage('eagleeye-panels', panelSettings)
    ...
    App->>localStorage: loadFromStorage('eagleeye-panels')
