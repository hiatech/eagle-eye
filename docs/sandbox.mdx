...
description: "Exercise the Eagle Eye API with no key and no quota: deterministic sandbox fixtures, anonymous MCP discovery, and a read-only data surface for safe testing."
---

Eagle Eye provides a sandbox so agents and integrations can be built and tested **without an API key, without consuming quota, and without any risk to production data**. It has three legs:
...
```bash
curl https://www.eagle-eye.app/sandbox/index.json
```
...
```bash
curl https://www.eagle-eye.app/sandbox/get-resilience-score.json
```
...
```json
{
  ...
  "operation": {
    ...
    "productionUrl": "https://api.eagle-eye.app/api/resilience/v1/get-resilience-score"
  },
...
- **Schema-valid** — every `response.body` validates against the operation's response schema in [openapi.json](https://eagle-eye.app/openapi.json).
...
The production MCP server at `https://eagle-eye.app/mcp` lets you explore the full tool surface with no auth and no daily-quota consumption:
...
The [docs MCP server](/mcp-overview) at `https://www.eagle-eye.app/docs/mcp` is entirely public — search and read this documentation over MCP with no key at all.

## 3. A read-only data surface

Every data operation in the [REST API](/api-reference) and every MCP data tool is **read-only**: there is no call an agent can make against `api.eagle-eye.app` that mutates production data. The only mutating surfaces are account-scoped (API-key management, alert rules, notification channels) and require an authenticated session — they are deliberately absent from the sandbox.

## Going live

1. Issue a key at [eagle-eye.app/pro](https://eagle-eye.app/pro) and send it as `X-EagleEye-Key: wm_<40-hex>` (or use [OAuth 2.1](/api-oauth) with `scope=mcp`).
