---
title: "Changelog"
description: "All notable changes to Eagle Eye organized by release — new features, bug fixes, API additions, and breaking changes across every version."
rss: true
---

All notable changes to Eagle Eye are documented here. Subscribe via [RSS](https://www.eagle-eye.app/docs/changelog/rss.xml) to stay updated.
...
- **CDN cache bypass closed**: `CDN-Cache-Control` header now only emitted for trusted origins (eagle-eye.app, Vercel previews, Tauri). No-origin server-side requests always invoke the edge function so `validateApiKey` runs, preventing a cached 200 from being served to external scrapers.
...
- Story header uses full domain EAGLE-EYE.APP (#799)
...
- **Edge caching**: Cloudflare edge caching for proxy.eagle-eye.app (#478) and api.eagle-eye.app (#471)
...
- **Finance variant**: Added a dedicated market-first variant (`finance.eagle-eye.app`) with finance/trading-focused feeds, panels, and map defaults
...
- **CORS hardening**: Tighten Vercel preview deployment regex to block origin spoofing (`eagleeyeEVIL.vercel.app`)
