...
- **Pro** — sign in with your EagleEye account, no key to manage. 50 quota-consuming calls per UTC day.
...
Upgrade or generate a key at [eagle-eye.app/pro](https://www.eagle-eye.app/pro). The rest of this guide assumes Claude Desktop + OAuth (the simplest path); if you'd rather paste a `wm_…` key into a `curl` script, jump to [Server-side curl](#server-side-curl) at the bottom.

## 2. Add the server to Claude Desktop

Edit `~/Library/Application Support/Claude/claude_desktop_config.json` (macOS) or the Windows equivalent at `%APPDATA%\Claude\claude_desktop_config.json` and add the `eagleeye` entry:
...
Restart Claude Desktop. The first time you mention EagleEye in a chat, Claude pops the OAuth consent screen — click **Sign in with EagleEye Pro**, authenticate in your browser, and the token is stored locally by Claude. No API key ever touches your filesystem.
...
```json
{
  "mcpServers": {
    "eagleeye": {
      "command": "npx",
      "args": ["-y", "mcp-remote", "https://eagle-eye.app/mcp", "--header", "X-EagleEye-Key:wm_YOUR_KEY"]
    }
  }
}
```
...
```bash
export WM_KEY="wm_0123456789abcdef0123456789abcdef01234567"  # API Starter+ key from eagle-eye.app/settings
...
The `wm_…` user API key goes in `X-EagleEye-Key`, **not** as a `Bearer` token — sending it as a bearer fails OAuth resolution and returns `401 invalid_token`. If you already have an OAuth access token from `/api/oauth/token`, use `Authorization: Bearer $TOKEN` instead and drop the `X-EagleEye-Key` header.
...
- **[Command-line client](/cli)** — prefer a shell? `npx eagleeye tools` drives the same tools from your terminal or a script, no integration to write.
- **[Official SDKs](/sdks)** — prefer a library? Zero-dependency clients for Python (`pip install eagleeye-sdk`), Ruby (`gem install eagleeye`), and Go (`go get github.com/hiatech/eagle-eye/sdk/go`) call the same tools with language-native helpers.
- **[Authentication overview](/authentication)** — when to use an API key in `X-EagleEye-Key` vs. OAuth vs. browser session.
