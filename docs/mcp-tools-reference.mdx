...
description: "Per-tool reference for every MCP tool Eagle Eye exposes — parameters, freshness windows, backing API endpoints, and example call payloads."
---
...
If you've completed the OAuth flow instead, replace `-H "X-EagleEye-Key: $WM_KEY"` with `-H "Authorization: Bearer $TOKEN"` in each example.
...
Real-time equity quotes, commodity prices (including gold futures GC=F), crypto prices, forex FX rates (USD/EUR, USD/JPY etc.), sector performance and valuation coverage, ETF flows, and Gulf market quotes from EagleEye's curated bootstrap cache.
...
- **Kind:** live RPC — proxies a fetch to the EagleEye API on each call. Edge-runtime timeout: **8.0s**.
...
- **Kind:** live RPC — proxies a fetch to the EagleEye API on each call. Worst-case total budget **~24s** (2s context-digest fetch + 22s brief generation, sequential).
...
AI-classified geopolitical threat news summaries, GDELT intelligence signals, cross-source signals, and security advisories from EagleEye's intelligence layer.
...
This is a deliberate, documented posture rather than an oversight; the full decision, and the operator path for retracting a specific record, are in [`docs/architecture/intel-history-untrusted-text.md`](https://github.com/hiatech/eagle-eye/blob/main/docs/architecture/intel-history-untrusted-text.md).
</Warning>
...
- **Kind:** live RPC — proxies a fetch to the EagleEye API on each call. Edge-runtime timeout: **25.0s**.
...
AI-generated geopolitical and economic forecasts from EagleEye's predictive models. Covers upcoming risk events and probability assessments.
