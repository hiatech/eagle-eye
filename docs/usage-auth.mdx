...
description: "Authenticate against Eagle Eye with browser session, API key, OAuth bearer, or Clerk — plus how server-side gating enforces plan entitlements."
---

EagleEye has four authentication modes. Which one applies depends on how you're calling.
...
| **API key** | `X-EagleEye-Key: wm_0123456789abcdef0123456789abcdef01234567` | Server-to-server, scripts, SDKs | User API keys cover entitled API access; operator-issued enterprise keys cover internal/partner access. |
| **OAuth bearer** | `Authorization: Bearer <oauth-token>` | MCP clients (Claude, Cursor, Inspector) | `/api/mcp`. The handler also accepts a direct `X-EagleEye-Key` in lieu of an OAuth token — see [MCP](/mcp-overview#authentication). |
...
For these, you **must** send an API key; `X-EagleEye-Key` is the canonical header.
...
<Warning>
**A Cloudflare Worker** (`api-cors-preflight`) is the authoritative CORS handler for `api.eagle-eye.app` — it overrides `_cors.js` and `vercel.json`. If you're changing origin rules, change them in the Cloudflare dashboard.
</Warning>
...
`X-EagleEye-Key` is the canonical header. API-key-authenticated endpoints also accept `X-Api-Key` as an alias for compatibility with generic API clients, including standalone edge functions that use `validateApiKey()` and gateway-backed routes. Do not send user API keys as bearer tokens or query-string parameters unless an endpoint explicitly documents that form.

For `/api/bootstrap`, server-side callers should use `https://api.eagle-eye.app/api/bootstrap` with one of these API-key headers. There is no separate gateway host, token-exchange step, activation step, or IP allow-list requirement for standard server-to-server access. The endpoint's anonymous weather path (`?keys=weatherAlerts`) is public **only when no key header is sent** — once you attach `X-EagleEye-Key`/`X-Api-Key`, the request is validated even for weather, so a key without current API access returns `403` rather than falling back to anonymous data. That URL is served `no-store` precisely so this holds: it is uncached, so an invalid key always reaches the origin. If you want the CDN-cached weather read instead, use the explicitly public `?keys=weatherAlerts&public=1`, which returns the same shared payload to every caller and ignores credentials by design.

### Server-side validation

The edge function calls `validateApiKey(req, { forceKey?: boolean })`:

1. Desktop origins must send an enterprise key in `X-EagleEye-Key`.
2. If `forceKey` is false, a valid `wms_` browser session cookie satisfies the anonymous/public gate.
3. Enterprise keys are checked against `EAGLEEYE_VALID_KEYS`.
...
- A valid `X-EagleEye-Key` (env-allowlisted from `EAGLEEYE_VALID_KEYS`, or a user-owned `wm_`-prefixed key whose Convex record has the `apiAccess` entitlement), **or**
- A Clerk `Authorization: Bearer …` token whose user has role `pro` or Dodo entitlement tier ≥ 1.

From the browser, `premiumFetch()` (`src/services/premium-fetch.ts`) handles this by injecting one of those credentials on every request. Desktop app uses `EAGLEEYE_API_KEY` from the runtime config. Server-to-server callers must send the header explicitly.
