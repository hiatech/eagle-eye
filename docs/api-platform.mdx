...
description: "Bootstrap, health, version, cache-purge, and user-preference endpoints — the plumbing every Eagle Eye client talks to on startup and cleanup."
---
...
- **Server-to-server**: call `https://api.eagle-eye.app/api/bootstrap` directly with `X-EagleEye-Key: wm_...`. There is no separate gateway host, token exchange, activation step, or IP allow-list requirement for this endpoint.
...
each supported value resolves to the same Eagle Eye asset for the requested
...
- `X-EagleEye-Desktop-Timestamp`: Unix epoch milliseconds, within 5 minutes of server time.
