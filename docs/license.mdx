...
description: "Eagle Eye licensing terms — AGPL-3.0-only source license, optional commercial licensing, and the separate trademark and branding policy."
---

Eagle Eye source code is licensed under the [GNU Affero General Public License v3.0 (AGPL-3.0-only)](https://www.gnu.org/licenses/agpl-3.0.html).

This page is a plain-language summary for contributors and users. It is not legal advice and does not replace the license text in [`LICENSE`](https://github.com/hiatech/eagle-eye/blob/main/LICENSE).
...
**Exception — official client packages are MIT.** The thin API clients — the [`eagleeye` npm CLI](https://www.npmjs.com/package/eagleeye) (`cli/`), and the [Python](https://pypi.org/project/eagleeye-sdk/) (`sdk/python/`), [Ruby](https://rubygems.org/gems/eagleeye) (`sdk/ruby/`), and [Go](https://pkg.go.dev/github.com/hiatech/eagle-eye/sdk/go) (`sdk/go/`) SDKs — are licensed under MIT (see the `LICENSE` file in each directory) so they can be embedded in any application without copyleft obligations. This exception covers only those client packages: the Eagle Eye platform, server, and dashboard remain AGPL-3.0-only.
...
- use Eagle Eye for personal, educational, research, nonprofit, or commercial purposes
...
- combine Eagle Eye code into proprietary products in ways the AGPL would not permit
...
Contact the maintainer through the [GitHub repository](https://github.com/hiatech/eagle-eye) to discuss commercial licensing.
...
The AGPL covers copyright permissions for the code. It does not grant rights to use the Eagle Eye name, logo, visual identity, or official project branding.
...
**Using Eagle Eye's public API in your own tool**: API terms of service may apply separately from the source-code license.
