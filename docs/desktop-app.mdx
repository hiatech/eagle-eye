...
description: "Tauri desktop architecture for Eagle Eye — sidecar process management, secret storage, cloud fallback, and multi-platform build pipelines."
---
...
- **Cloud fallback** — when a local API handler fails or is missing, requests transparently fall through to the cloud deployment (eagle-eye.app) with origin headers stripped
...
Eagle Eye desktop uses a runtime configuration schema with per-feature toggles and secret-backed credentials.
...
- `OLLAMA_API_URL`
- `OLLAMA_MODEL`
- `EAGLEEYE_API_KEY` — gates cloud fallback access (min 16 chars)
- `WTO_API_KEY`
- `AVIATIONSTACK_API`
- `ICAO_API_KEY`
- `UCDP_ACCESS_TOKEN`
...
- Eagle Eye API key: cloud fallback is blocked; desktop operates local-only.
...
The desktop app checks for new versions by polling `eagle-eye.app/api/version` — once at startup (after a 5-second delay) and then every 6 hours. When a newer version is detected (semver comparison), a non-intrusive update badge appears with a direct link to the GitHub Release page.

Update prompts are dismissable per-version — dismissing v2.5.0 won't suppress v2.6.0 notifications. One Eagle Eye binary ships and every variant is selected in-app, so the updater resolves the download by OS and architecture alone (`/api/download?platform=…`) rather than by variant.
