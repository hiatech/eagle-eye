...
description: "Static map assets, country geometry, forward and reverse geocoding services, and boundary overrides used across Eagle Eye's map layers."
---

## R2 CDN — `maps.eagle-eye.app`

All large static map files are served from Cloudflare R2, **not** from Vercel. The R2 bucket `eagleeye-maps` is fronted by a CF-proxied custom domain:

| URL | Use |
|-----|-----|
| `https://maps.eagle-eye.app/<file>` | Production URL (CF-proxied, cached, CORS headers) |
...
```bash
# Single file
rclone copyto <local-path> r2:eagleeye-maps/<filename>
...
2. `country-boundary-overrides.geojson` — optional higher-resolution polygons from [Natural Earth](https://www.naturalearthdata.com/), served from R2 CDN (`maps.eagle-eye.app`). Features matched by `ISO3166-1-Alpha-2` (or `ISO_A2`) code; matching features replace the base geometry
...
4. Upload to R2:
   ```bash
   rclone copyto public/data/country-boundary-overrides.geojson r2:eagleeye-maps/country-boundary-overrides.geojson
   ```
...
PMTiles are also served from R2 via `maps.eagle-eye.app`, configured through `VITE_PMTILES_URL`.

## Common Mistakes

| Mistake | Fix |
|---------|-----|
| Using `pub-*.r2.dev` URLs in code | Always use `maps.eagle-eye.app` (CF-proxied) |
