...
<Info>
**Pro and API tiers can both connect via OAuth — no API key required.** Pro subscribers click _"Sign in with EagleEye Pro"_ on the consent page; API Starter / Business / Enterprise users may sign in the same way OR paste a `wm_…` key. Free-tier users see a `403 INSUFFICIENT_TIER` at the OAuth step.
</Info>
...
| Endpoint | Purpose |
|----------|---------|
| `https://eagle-eye.app/mcp` | JSON-RPC server (Streamable HTTP transport; JSON responses by default, SSE responses when clients advertise `text/event-stream`; `initialize` defaults to protocol `2025-03-26`) |
| `https://api.eagle-eye.app/api/oauth/register` | Dynamic Client Registration (RFC 7591) |
| `https://api.eagle-eye.app/api/oauth/authorize` | OAuth 2.1 authorization endpoint (PKCE required) |
| `https://api.eagle-eye.app/api/oauth/token` | Token endpoint (authorization_code + refresh_token) |
| `https://api.eagle-eye.app/.well-known/oauth-authorization-server` | AS metadata (RFC 8414) |
| `https://eagle-eye.app/.well-known/oauth-protected-resource` | Resource server metadata (RFC 9728) |

Server identifier: `eagleeye` v1.15.0.

Registry listings: the server is published in the [official MCP registry](https://registry.modelcontextprotocol.io/v0/servers?search=eagleeye) as `app.eagleeye/mcp` — a domain-verified namespace, so clients that resolve servers through the registry get the same endpoint and metadata as the server card above — and listed on [Smithery](https://smithery.ai/servers/eagleeye/wm-mcp) and [mcp.so](https://mcp.so/server/eagle-eye).

### Protocol negotiation

EagleEye's static server card at `/.well-known/mcp/server-card.json` advertises protocol version `2025-06-18`, and the live `initialize` handshake negotiates it by default — so the advertised floor and the negotiated version stay in lock-step:
...
EagleEye supports the Streamable HTTP POST flow with either JSON or SSE responses:
...
Pro subscribers (and API Starter+ users who'd rather not paste a key) can authorize MCP clients via their existing EagleEye account — no API key needed.

1. **Add the server URL** in your MCP client. The canonical entrypoint is:

   ```
   https://api.eagle-eye.app/mcp
   ```

   (`https://eagle-eye.app/mcp` works too — it proxies the same handler.)

2. **Click "Sign in with EagleEye Pro"** on the consent page. This is the default CTA. You'll bounce through `eagle-eye.app/mcp-grant` (Clerk-protected — sign in if needed), then back to `api.eagle-eye.app/oauth/authorize-pro`, then to your client's redirect.
...
- Manage connected clients in **[Settings → Connected MCP clients](https://eagle-eye.app/settings)**.
...
|------|------------|------------|-----------------------|-------|
| **Free** | ❌ No | — | — | OAuth flow returns `403 INSUFFICIENT_TIER`. Upgrade at [eagle-eye.app/pro](https://www.eagle-eye.app/pro). |
...
EagleEye also surfaces current MCP plan-limit notices in Settings and sends a bounded-cadence email when a paid user approaches or exceeds their plan allowance. Notices are informational and action-oriented: they offer retry/reset guidance, checkout when the next tier is self-serve, or support contact when it is not. They do not automatically upgrade the account or create overage charges.
...
In addition to the 59 tools, EagleEye exposes MCP prompts and resources so clients can discover common workflows and address stable data slices without inventing tool plans from scratch.
...
|--------------|--------------|
| `eagleeye://seed-meta/freshness` | Stock market-data bootstrap write-age only: `cached_at` and `stale` for `seed-meta:market:stocks`. Does **not** certify sector valuation completeness — use `get_market_data` `valuationCoverage` (`sourceStatus`, unavailable/last-good/diagnostics). Cheap seeder health probe — no auth, no quota. |
...
|-----------------------|--------------|
| `eagleeye://countries/{iso2}/risk` | Country risk score, component breakdown, travel advisory, and sanctions exposure. `{iso2}` is lowercase alpha-2, for example `de` or `us`. |
| `eagleeye://chokepoints/{slug}/status` | Chokepoint transit summary and risk narrative. `{slug}` is one of the published kebab-case chokepoint slugs, such as `suez`, `strait-of-hormuz`, or `bab-el-mandeb`. |
| `eagleeye://markets/{symbol}/quote` | Single-symbol market quote slice. `{symbol}` is uppercase, for example `AAPL`, `GC=F`, or `BTC-USD`. |
...
|-----------------|-------------|---------|
| `ui://eagleeye/country-risk.html` | `get_country_risk` | CII score, unrest/conflict/security/news component breakdown, travel-advisory level, and OFAC sanctions exposure. |
| `ui://eagleeye/world-brief.html` | `get_world_brief` | The precomputed, citation-grounded global intelligence brief as readable paragraphs, plus grounding headlines and source articles. |
| `ui://eagleeye/country-brief.html` | `get_country_brief` | The AI-synthesised per-country brief as paragraphs, with the analytical framework lens and grounding sources. |
| `ui://eagleeye/market-radar.html` | `get_market_data` | The Fear & Greed composite plus per-asset-class quote tables (equities, commodities, crypto, Gulf, sectors) with signed, colour-coded change. |
| `ui://eagleeye/chokepoint-monitor.html` | `get_chokepoint_status` | Per-chokepoint rolling transit summaries (today's count, week-over-week change, tanker split) with a risk-level badge. |
| `ui://eagleeye/news-intelligence.html` | `get_news_intelligence` | AI-classified top stories with category, alert flag, country, and source. |
| `ui://eagleeye/conflict-events.html` | `get_conflict_events` | Active armed-conflict events (belligerents, violence type, country, fatalities, date) from the UCDP feed. |
| `ui://eagleeye/natural-disasters.html` | `get_natural_disasters` | Recent earthquakes (USGS magnitude, place, time) and active wildfires (NASA FIRMS), grouped. |
| `ui://eagleeye/prediction-markets.html` | `get_prediction_markets` | Active event-contract odds grouped by category (geopolitical, tech, finance) with a probability bar per market. |
| `ui://eagleeye/forecasts.html` | `get_forecast_predictions` | AI-generated geopolitical and economic forecasts as probability cards (title, domain, region). |
...
Server-side with a direct API key — send it as `X-EagleEye-Key`, **not** as a bearer token.
...
Seed-level health per key: [status.eagle-eye.app](https://status.eagle-eye.app/).
...
- [Pro features](https://www.eagle-eye.app/pro)
