...
The previous architecture served embeds from `https://eagle-eye.app/api/youtube/embed`. The parent was also `https://eagle-eye.app`. **Same origin** = WKWebView's autoplay policy respected the `.none` setting. When we moved embeds to the local sidecar (`http://localhost:PORT`), the parent became `tauri://localhost` — now cross-origin, breaking autoplay.
