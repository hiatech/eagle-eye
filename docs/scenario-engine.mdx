...
Templates are defined in `server/eagleeye/supply-chain/v1/scenario-templates.ts`. Each template has a `type` drawn from a small, curated set so scenarios are browsable by category rather than a free-form list.
...
Scenario Engine is **PRO**. Free users see the trigger buttons but are blocked at activation: a `scenario-engine` gate-hit event is logged and the map is not repainted. The `ScenarioService.RunScenario` handler also enforces PRO at the edge (`server/eagleeye/scenario/v1/run-scenario.ts`).
...
- **Scenario templates** — `server/eagleeye/supply-chain/v1/scenario-templates.ts`. Additions require a proto-side change; not user-configurable today.
