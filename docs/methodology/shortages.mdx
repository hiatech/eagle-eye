...
description: "How Eagle Eye classifies and publishes fuel shortage alerts (jet, petrol, diesel, heating oil) using an automated evidence-threshold pipeline."
---
...
EagleEye runs every classifier fully automated. Shortage tiers are no exception. Quality comes from **tiered evidence thresholds + an LLM second-pass sanity check + auto-decay of stale rows**, not from a human reviewer.
...
Every public shortage row exposes its `evidenceSources[]` inline — you can read the sources the classifier used. This is what makes EagleEye's shortage registry defensible without an editorial queue.
...
[GitHub issues](https://github.com/hiatech/eagle-eye/issues); they
...
today via [GitHub issues](https://github.com/hiatech/eagle-eye/issues).
