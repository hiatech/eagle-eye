...
(proto `eagleeye.intelligence.v1.GetRiskScoresResponse`).

These weights are **editorial** — authored by the EagleEye intelligence
...
> `server/eagleeye/intelligence/v1/_risk-config.ts`, an update to this
...
`server/eagleeye/intelligence/v1/get-risk-scores.ts`. Each `CiiScore`
therefore exposes:
...
[`shared/cii-weights.ts`](https://github.com/hiatech/eagle-eye/blob/main/shared/cii-weights.ts); server and frontend
