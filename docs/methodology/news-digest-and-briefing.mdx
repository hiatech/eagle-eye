...
description: "How EagleEye collects RSS/Atom feeds, classifies news, scores importance, tracks stories, filters briefs, uses LLMs, and controls bias."
---

## Scope

This page is the canonical public methodology for the news-feed digest,
daily digest notifications, and EagleEye Brief. It covers the automated
...
Feed inventory lives in `server/eagleeye/news/v1/_feeds.ts`.
...
`energy` is a site and client-feed variant (`energy.eagle-eye.app`), but it
...
`server/eagleeye/news/v1/_classifier.ts`.
...
EagleEye Brief uses two LLM surfaces:
...
- Feed inventory and digest build: `server/eagleeye/news/v1/_feeds.ts`,
  `server/eagleeye/news/v1/list-feed-digest.ts`
- Keyword classifier: `server/eagleeye/news/v1/_classifier.ts`
- API contract: `proto/eagleeye/news/v1/list_feed_digest.proto`
